...
// 献祭生命门槛（与 effects.js hp_sacrifice handler 一致）：hp > 代价才可施放
// （否则选了也会被 handler 阻断，白耗回合）
export function sacrificeCastable(unit, skill) {
  for (const e of skill.effectRounds) {
    if (e.type !== 'hp_sacrifice') continue;
    const base = Number.isFinite(unit.maxHp) ? unit.maxHp : unit.hp;
    const cost = Math.max(1, Math.round(base * (e.costPct || 0)));

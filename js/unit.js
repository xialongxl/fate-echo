...
export class CombatUnit {
  ...
  heal(amount) {
    ...
    // 受疗乘区：海洋领域 +20%（healingMult）× 受疗降低 debuff（heal_cut，2026-08-12 敌人技能从零重做新增）
    const healCut = typeof this.statBonus === 'function' ? Math.min(80, Math.max(0, this.statBonus('heal_cut_pct'))) : 0;
    const hMult = (Number.isFinite(this.healingMult) ? Math.max(0, this.healingMult) : 1) * (1 - healCut / 100);

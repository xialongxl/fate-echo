...
// 存档结构（v10，2026-08-11 删 MP）：
//   { version, player: {level, exp, gold, hp, equipment: 8槽对象,
//     inventory: 装备对象数组, finaleEssence, finaleCollection: 8槽bool,
//     redPity（红装保底 0.5 精度）, battleSeed（战斗种子 防 SL）,
//     collections（收藏品 ID 数组，局内）, skillLoadout（技能配置）},
//     tower: {depth, floor}, area: 图结构探索区 }
// ============================================================

import { SLOTS, GEAR_RARITY } from './equipment.js';
import { COLLECTIONS } from './collections.js';
import { AREAS_PER_RUN } from './area.js';
import { SKILLS_DB } from './data.js';

// 技能 id 白名单（技能配置清洗用；data.js 纯数据无依赖，无循环）
const SKILL_IDS = new Set(SKILLS_DB.map((s) => s.id));
// 终焉技能 id（desc 含 [终焉]；配置迁移分流用）
const FINALE_IDS = new Set(SKILLS_DB.filter((s) => /\[终焉\]/.test(s.desc || '')).map((s) => s.id));
...
export function defaultGameState() {
  return {
    ...
    player: {
      ...
      collections: [], // 收藏品 ID 数组（局内：存档跟随，newGame 清空；黑流树海同款）
      skillLoadout: null, // 技能配置 { regular: [≤5], finale: [≤3] }；null = 全部解锁（局外技能配置面板）
    },
...
// v5 → v6 迁移：收藏品系统（局内）补默认 collections
MIGRATIONS[5] = (d) => {
  const player = d.player && typeof d.player === 'object' ? { ...d.player } : {};
  if (!Array.isArray(player.collections)) player.collections = [];
  return { ...d, player };
};

// v6 → v7 迁移：int → def 统一（2026-08-11，末光确认 int 是内部字段遗留，
//   防御力统一 def；装备 stats 的 int 字段并入 def，玩家无 int 字段）
MIGRATIONS[6] = (d) => {
  const migrateGear = (g) => {
    if (!g || typeof g !== 'object' || !g.stats || typeof g.stats !== 'object') return g;
    const stats = { ...g.stats };
    if (Number.isFinite(stats.int)) {
      if (stats.int >= 0) {
        stats.def = Number.isFinite(stats.def) ? stats.def + stats.int : stats.int; // int 并入 def（防御力）
      } // 负 int 无防御意义：不并入，仅清除
      delete stats.int;
    }
    return { ...g, stats };
...
// v7 → v8 迁移：技能配置（局外可配置技能，2026-08-11）补默认 skillLoadout = null（全部解锁）
MIGRATIONS[7] = (d) => {
  const player = d.player && typeof d.player === 'object' ? { ...d.player } : {};
  if (player.skillLoadout === undefined) player.skillLoadout = null;
  return { ...d, player };
};

// v8 → v9 迁移：技能配置结构 {main, instant} → {regular, finale}（5 常规 + 3 终焉，
//   2026-08-11 用户定案）：main+instant 合并 → regular（非终焉分流），终焉技能 → finale
MIGRATIONS[8] = (d) => {
  const player = d.player && typeof d.player === 'object' ? { ...d.player } : {};
  const old = player.skillLoadout;
  if (old && typeof old === 'object' && (Array.isArray(old.main) || Array.isArray(old.instant))) {
    const merged = [...(Array.isArray(old.main) ? old.main : []), ...(Array.isArray(old.instant) ? old.instant : [])];
    player.skillLoadout = {
      regular: merged.filter((id) => typeof id === 'string' && !FINALE_IDS.has(id)),
      finale: merged.filter((id) => typeof id === 'string' && FINALE_IDS.has(id)),
    };
  }
  return { ...d, player };
};

// v9 → v10 迁移：删除 MP（2026-08-11 用户定案，AP 回转体系）——player.mp 字段丢弃
MIGRATIONS[9] = (d) => {
  const player = d.player && typeof d.player === 'object' ? { ...d.player } : {};
  delete player.mp;
  return { ...d, player };
...
// 技能配置白名单（2026-08-11 v9：5 常规 + 3 终焉结构）：
//   合法 = { regular: string[], finale: string[] }（id 字符串过滤 + 去重 + 上限 64 防膨胀 +
//   SKILLS_DB 存在性校验——防手工伪造存档软锁战斗）
//   畸形/缺结构 → null（全部解锁，现状兼容）；空 regular → null（至少保留 1 个常规技能）
//   注：5/3 上阵上限由配置面板 UI 守卫（添加时拒绝），sanitize 不截断——
//      null→全选路径可超上限（与"全部解锁"现状行为一致），读档不丢技能
function sanitizeSkillLoadout(v) {
  if (!v || typeof v !== 'object') return null;
  const clean = (arr) => Array.isArray(arr)
    ? [...new Set(arr.filter((x) => typeof x === 'string' && x.length <= 32 && SKILL_IDS.has(x)))].slice(0, 64)
    : [];
  const regular = clean(v.regular);
  const finale = clean(v.finale);
  if (!regular.length) return null;
  return { regular, finale };
}

// 装备对象白名单（逐字段校验；畸形/未知槽位 → null）
// v4（末光照抄）：属性 atk/def/haste/crit/versa + baseScore/orbs/精炼字段
function sanitizeGear(g) {
  if (!g || typeof g !== 'object' || !SLOTS.includes(g.slot)) return null;
  const stats = {};
  for (const k of ['atk', 'def', 'crit', 'haste', 'versa']) {
  ...
  return {
    ...
    orbs: Array.isArray(g.orbs) ? g.orbs.slice(0, 3).map((o) => (typeof o === 'string' && o.length <= 32 ? o : null)) : [], // R2-B：字符串长度守卫防膨胀
...
export function sanitizeSave(raw) {
  ...
  const player = {
    ...
    collections: Array.isArray(p.collections) ? [...new Set(p.collections.filter((c) => typeof c === 'string' && COLLECTIONS.some((x) => x.id === c)))].slice(0, 50) : [], // 收藏品（局内，白名单：合法 id + 去重 + 上限 50）
    skillLoadout: sanitizeSkillLoadout(p.skillLoadout), // 技能配置（null = 全部解锁；主技能至少 1 个，防战斗无技能卡死）
  };
...
function createIndexedDBStore() {
  ...
  const dbPromise = new Promise((resolve, reject) => {
    ...
    // 升级被其他标签页的旧连接阻塞时不得永久挂起（否则存档界面空白/假保存）
    req.onblocked = () => reject(new Error('IndexedDB 升级被其他标签页阻塞：请关闭其他 Fate_echo 页面后重试'));
  });
...
// 轮次/层数以 area（黑流树海图结构）为准：tower 为 v2 遗留字段从不推进，
//   实际游戏推进 area.depth（main.js openExit），层数 = (depth-1)×3 + areaIndex + 1
function metaOf(id, state) {
  ...
  return {
    ...
    depth: a.depth || t.depth || 1,
    floor: a.depth ? (a.depth - 1) * AREAS_PER_RUN + (a.areaIndex || 0) + 1 : (t.floor || 1),
  };
}

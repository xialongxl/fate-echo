...
export class OverworldUI {
  ...
  renderSkillConfig(game) {
    ...
    //   {dmg} 约伤害（无减伤近似）、{heal} 治疗、{shieldVal} 护盾、{mpRecover} 回蓝；
    //   "冷却{cd}秒" → 保留 CD 的技能显示 X 回合、其余"无冷却"；"持续{dur}秒" → X 层；
    //   "每秒" → "每层"；终焉技能附加"（终焉：消耗全部行动点）"
    const u = makeBattlePlayerRef(game).unit;
    const fillDesc = (s) => {
      const e0 = (s.effects || [])[0] || {};
      let d = (s.desc || '')
        .replace(/冷却\{cd\}秒/g, s.cd > 0 ? `冷却${Math.max(1, Math.round(s.cd / 2500))}回合` : '无冷却')
        .replace(/持续\{dur\}秒/g, `持续${Math.max(0, Math.round(e0.dur || 0))}层`)
        .replace(/每秒/g, '每层')
      ...
      const vars = {
        ...
        mpRecover: e0.type === 'mp_recover_pct' ? Math.round((e0.val || 0) * 100) : 0,
    ...
    this.renderPanel(`⚙ 技能配置（常规 ${regularOn}/5 · 终焉 ${finaleOn}/3）`, (panel) => {
      ...
      const mkGroup = (title, list, sel, group) => {
        ...
        for (const s of list) {
          ...
          info.style.flex = '1';
          // 摘要行（倍率/MP/CD 回合——战斗数值速览）+ 完整描述行（模板填充）
          info.innerHTML = `<b>${esc(s.name)}</b> <span class="node-sub">Lv${s.reqLv}解锁${s.dmgMult > 0 ? ` · 倍率 ×${s.dmgMult}` : ''}${s.cost > 0 ? ` · ${s.cost}MP` : ''}${s.cd > 0 ? ` · CD ${Math.round(s.cd / 2500)}回合` : ''}</span><div class="node-sub" style="margin-top:2px;">${esc(fillDesc(s))}</div>`;
          row.appendChild(info);
          const btn = document.createElement('button');
  ...
  renderGlossary(game, stats = null) {
    this.renderPanel('📖 术语表 · 帮助', (panel) => {
      ...
      const terms = [
        ...
        ['回合制', '1 回合 ≈ 2.5 秒；持续效果为层数制（每回合 -1 层，减完结束）'],
        ['冷却（CD）', '2026-08-11 已去 CD：非终焉技能无冷却（可每回合重复施放）；仅回蓝技保留 CD；终焉技能 = 消耗全部行动点（本回合独占行动）'],

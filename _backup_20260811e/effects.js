...
export const EFFECT_HANDLERS = {
  ...
  // 刷新规则（去 CD 配套，NEW_GAME.md §2.5"同类刷新取最大值"）：同 stat 且有限层数条目
  ...
  // 层数制刷新：同 stateName 条目刷新层数不叠加（去 CD 配套——否则每回合挂 dot 无限叠）
  ...
  //   2026-08-11 去 CD 配套：同来源（key=skill.id）刷新不叠加（hp/turns 取最大），
  //   不同来源仍叠加（A6 语义保留）

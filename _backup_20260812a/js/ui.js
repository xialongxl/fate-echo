...
export class BattleUI {
  ...
  _renderSkills() {
    ...
    const mkBtn = (skill, keyLabel, isInstant) => {
      ...
      const apTag = skill.isFinale ? '全 AP' : (skill.isInstant ? '' : '1 AP'); // 终焉消耗全部行动点/主技能 1 AP/瞬发无
...
export class OverworldUI {
  ...
  renderSkillConfig(game) {
    ...
    this.renderPanel(`⚙ 技能配置（常规 ${regularOn}/5 · 终焉 ${finaleOn}/3）`, (panel) => {
      const hint = document.createElement('div');
      hint.className = 'panel-text';
      hint.textContent = '点击切换上阵（即时保存）；常规至少保留 1 个、上限 5 个；终焉上限 3 个（消耗全部行动点）；未解锁灰色；被动自动生效；新游戏重置为全部解锁。';
      panel.appendChild(hint);
      ...
      mkGroup('── 终焉技能（上限 3：消耗全部行动点）──', finales, selectedFinale, 'finale');
      const note = document.createElement('div');
  ...
  renderGlossary(game, stats = null) {
    this.renderPanel('📖 术语表 · 帮助', (panel) => {
      ...
      const terms = [
        ['行动点 AP', '主技能消耗 1 点；每回合保底 1（回转收益留存）；每 10% 急速 +1 上限（默认 1，最高 3，防御姿态 +1 可超上限）'],

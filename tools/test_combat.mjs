...
//   持续效果结算（dot/hot/buff/shield/vuln 层数递减）、
...
const mkEnemy = (o = {}) => new CombatUnit(Object.assign(
  { name: '靶子', hp: 300, maxHp: 300, mp: 100, maxMp: 100, atk: 15, def: 10, level: 5, critChance: 0 },
  o,
));
// 技能数据列表（按 id）
...
// 数值预期：玩家 atk20 对 def10/lv5 敌人（减伤 10/210≈4.76%）
...
console.log('== B. 行动点 / 瞬发槽 ==');
{
  const e = eng();
  e.startTurn();
  t('B1 基础 apMax=1，第二主技能拒绝（s01 回 AP 净 0 除外）', e.queueMain('s03').ok && !e.queueMain('s04').ok);
  t('B2 拒绝原因：行动点不足', e.queueMain('s04').reason === '行动点不足');
}
{
  const e = eng();
  ...
  e.startTurn();
  t('B6 总急速45（15+30）→ 1+4=5（上限 8）', e.apMax === 5);
  e.player.buffs.push({ stat: 'haste', val: 50, turns: 99 });
  e.startTurn();
  t('B7 总急速95 → min(8, 1+9)=8（上限 8 触顶）', e.apMax === 8);
}
{
  const e = eng();
  e.startTurn();
...
console.log('== C. 施放与消耗 ==');
{
  ...
  e.queueMain('s03');
  e.confirm();
  t('C1 s03 施放无 MP 消耗（MP 已删，mp 恒默认）', e.player.mp === 100, `mp=${e.player.mp}`);
  t('C2 s03：伤害 34（1.8×20×减伤，floor）', e.enemies[0].hp === 300 - expectDmg(1.8), `hp=${e.enemies[0].hp}`);
  t('C3 s03 跨回合可重复施放（技能无使用限制）', e.turn === 2 && e.queueMain('s03').ok);
  t('C4 s01 使用后可连发（技能无使用限制）', e.queueMain('s01').ok && e.confirm() && e.queueMain('s01').ok);
}
{
  const e = eng();
  e.startTurn();
  t('C5 主技能 + 瞬发可同回合（瞬发不占 AP）', e.queueMain('s03').ok && e.queueInstant('s13').ok);
}
{
  // 技能无使用限制（2026-08-11 用户定案）：s07 跨回合可重复入队施放
  const e = eng();
  e.startTurn();
  e.queueInstant('s07');
  e.confirm();
  e.startTurn();
  t('C6 无使用限制：s07 下回合可再次入队', e.queueInstant('s07').ok, `reason=${e.queueInstant('s07').reason}`);
  e.queueMain('s01');
  e.confirm();
  t('C7 s07 跨回合可再次入队（无使用限制）', e.queueInstant('s07').ok);
}
{
...
console.log('== E. 伤害公式边界 ==');
{
  ...
  const huge = mkEnemy({ def: 100000, level: 100 });
...
console.log('== F. 防御姿态 ==');
{
  ...
  e.queueMain('s03');
  e.defend(); // 防御清空队列
  t('F1 defend 清空待行动（s03 未施放）', e.player.mp === 100, `mp=${e.player.mp}`);
...
console.log('== G. AP 保底 ==');
{
  ...
  e.player.ap = 0;
  skip(e);
  t('G1 每回合保底 1 行动点（AP 回转，不自动回满）', e.turn === 2 && e.ap === 1 && e.apMax === 1, `ap=${e.ap}/${e.apMax}`);
}
...
}
{
  ...
  e.startTurn();
  e.confirm();
  t('H3 敌人技能正常施放（MP 已删，无资源限制）', e.player.hp === 200 - Math.floor(15 * 1.8 * 0.8), `hp=${e.player.hp}`);
}
{
...
}
{
  ...
  e.startTurn();
  skip(e);
  t('I3 空确认 = 跳过行动，正常流转', e.turn === 2 && e.phase === 'player' && e.ap === 1);
}
{
...
}
{
  ...
  e2.startTurn();
  // 终焉 s51：dot 先回合挂载，下回合领域（分回合流程——终焉主技能耗光 AP 的自然节奏）
  e2.queueMain('s04');
  e2.confirm(); // 立即 9 + dot tick 15
  e2.queueMain('s51');
  e2.confirm(); // 领域展开 → 回合结束 dot 15 + 领域 ×1.15 = 109
  t('J11 死亡领域：敌人带 dot → ×1.15（立即 9 + dot 两跳 30 + 领域 109）', e2.enemies[0].hp === 1000 - expectDmg(0.5) - 2 * expectDmg(0.8) - Math.floor(95 * 1.15), `hp=${e2.enemies[0].hp}`);
}
{
...
{
  // 2026-08-12 终焉不独占：s41 虚空化身（终焉主技能）与 s04 可同回合入队
  //   （终焉主技能施放时扣光 AP = 自然限制；ap=2 时两者都可施放）
  const e = engCustom(['s04', 's41'], { mp: 200, maxMp: 200 });
  e.player.buffs.push({ stat: 'haste', val: 15, turns: 99 });
  e.defendBonus = 1; // 防御 +1：ap=2（终焉不独占验证需要多 AP）
  e.startTurn();
  t('K9a 终焉 s41 入队', e.queueMain('s41').ok);
  t('K9b 终焉不独占：s04 与终焉同回合可入队', e.queueMain('s04').ok, e.queueMain('s04').reason);
  e.confirm();
  t('K9 s41 虚空化身：dot 强化 6 层（回合 1 结束 → 剩 5）', e.player.dotEnhanced === 5, `dotEnhanced=${e.player.dotEnhanced}`);
  // 下回合挂 dot → 强化期间 4 倍（频率×2 且伤害×2：立即 9 + 2×30=60）
  e.queueMain('s04');
  e.confirm();
  t('K10 dot 4 倍（终焉扣光 AP 后 s04 本回合被跳过——AP 闸门，下一回合挂 dot 双结算）', e.enemies[0].hp === 300 - expectDmg(0.5) - 2 * expectDmg(1.6), `hp=${e.enemies[0].hp}`);
}
{
  ...
  e.queueMain('s04');
  e.confirm();
  e.queueInstant('s38'); // 终焉瞬发入队（占瞬发槽，不吃 AP）
  t('K11 终焉瞬发不独占：main 仍可入队', e.queueMain('s04').ok, e.queueMain('s04').reason);
  e.confirm();
  t('K12 s38 急速 +50 ×2 层（回合 2 结束 → 剩 1）', e.player.buffs.some((b) => b.stat === 'haste' && b.val === 50 && b.turns === 1));
}
{
  // 2026-08-12：s39 终焉咏叹调已改瞬发（s47 下位替代）——不独占，与主技能同回合
  const e = engCustom(['s03', 's39'], { mp: 300, maxMp: 300 });
  e.startTurn();
  e.queueMain('s03');
  t('K12d s39 为瞬发：main 先入队后瞬发槽仍可入队', e.queueInstant('s39').ok, e.queueInstant('s39').reason);
  e.confirm();
  t('K12f 终焉回合敌人仍行动（战斗继续）', e.turn === 2 && e.enemies[0].alive);
}
{
  // 持续效果刷新（同源刷新不叠加）；haste 15 → AP 2；敌人 atk 0（不干扰盾量）
  const e = engCustom(['s02', 's04', 's13'], {}, { hp: 1000, maxHp: 1000, atk: 0 });
  e.player.buffs.push({ stat: 'haste', val: 15, turns: 99 });
  e.defendBonus = 1; // 防御姿态 +1：AP=2 排满主技能（s02+s04）
  e.startTurn();
  e.queueMain('s02');
  e.queueMain('s04');
  e.queueInstant('s13');
  e.confirm();
  // 回合 2：同技能再次施放 → buff/dot/盾刷新不叠加
  e.defendBonus = 1;
  e.startTurn();
  e.queueMain('s02');
  e.queueMain('s04');
  e.queueInstant('s13');
  e.confirm();
  t('K15 buff 同 stat 刷新不叠加（versa 仍 10）', e.player.buffs.filter((b) => b.stat === 'versa').length === 1 && e.player.statBonus('versa') === 10, JSON.stringify(e.player.buffs));
  t('K16 dot 同 stateName 刷新不叠加（单条，刷新 5 → 本回合结束 tick 4）', e.enemies[0].dots.filter((d) => d.stateName === '恶咒').length === 1 && e.enemies[0].dots[0].turns === 4, JSON.stringify(e.enemies[0].dots.map((d) => ({ s: d.stateName, t: d.turns }))));
  t('K17 盾同来源刷新不叠加（单条 40，敌人保底 1 点后 39）', e.player.shields.length === 1 && e.player.shield.hp === 39, JSON.stringify(e.player.shields));
}
{
  const e = engCustom(BASE.concat(['s45']));
  e.player.hp = 50;
  ...
  e.queueInstant('s45');
  e.confirm();
  t('K13 s45 命运轮转：回满 HP/AP（随后受击 12 → 188）', e.player.hp === 200 - 12 && e.ap === e.apMax, `hp=${e.player.hp} ap=${e.ap}/${e.apMax}`);
  t('K14 s45 增伤 +50% ×6 层（回合 1 结束 → 剩 5）', e.player.buffs.some((b) => b.stat === 'dmg_up_pct' && b.val === 50 && b.turns === 5));
}
{
  // 敌人终焉后跳过瞬发（2026-08-12 终焉不独占：此断言验证敌人终焉施放后玩家已死/战斗结束 → 不再放瞬发；注释修正——非终焉独占规则）
  const mkMetaEnemy = (ids, tier, eOpts = {}) => ({
    unit: mkEnemy(eOpts),
    skills: sk(ids),
    meta: { id: 'e_test', tier, enragePct: undefined, dotImmune: false, desc: '测试' },
  });
  const logs = [];
  const e1 = new CombatEngine({
    player: { unit: mkUnit({ hp: 500, maxHp: 500 }), skills: sk(['s01']) },
    enemies: [mkMetaEnemy(['s40', 's26'], 'elite', { hp: 1000, maxHp: 1000, atk: 5 })],
    rng,
  });
  e1.on('log', (x) => logs.push(x));
  e1.startTurn();
  skip(e1); // 敌人 s40 终焉献祭（一击击杀玩家）→ 跳过瞬发 s26
  t('K18 敌人终焉主行动后跳过瞬发（献祭后无 s26）', logs.some((l) => l.text.includes('献祭')) && !logs.some((l) => l.text.includes('暗影裂隙')), logs.map((l) => l.text).slice(0, 5).join('|'));
  const logs2 = [];
  const e2 = new CombatEngine({
    player: { unit: mkUnit({ hp: 500, maxHp: 500 }), skills: sk(['s01']) },
    enemies: [mkMetaEnemy(['s09', 's26'], 'elite', { hp: 1000, maxHp: 1000, atk: 5 })],
    rng,
  });
  e2.on('log', (x) => logs2.push(x));
  e2.startTurn();
  skip(e2);
  t('K19 非终焉主行动后仍施放瞬发（s09 + s26）', logs2.some((l) => l.text.includes('灵魂燃烧')) && logs2.some((l) => l.text.includes('暗影裂隙')), logs2.map((l) => l.text).slice(0, 6).join('|'));
}
{
  // 防御姿态 +1AP × 终焉瞬发（2026-08-12：终焉不独占——s39 瞬发与主技能共存）
  const e = engCustom(['s03', 's01', 's39'], { mp: 300, maxMp: 300 });
  e.startTurn();
  e.defend(); // 回合 2：apMax = 2
  t('K20 防御 +1AP：主技能占满后终焉瞬发 s39 仍可入队（瞬发槽）', e.queueMain('s03').ok && e.queueMain('s01').ok && e.queueInstant('s39').ok, `apMax=${e.apMax}`);
  e.confirm();
  t('K21 终焉瞬发回合正常推进', e.turn === 3);

{
  // AP 闸门显式断言（终检 F15）：终焉主技能扣光 AP → 后续非回 AP 主技能跳过 + 播报
  const e = engCustom(['s40', 's03'], {}, { hp: 9999, maxHp: 9999, atk: 0 }); // s40 灵魂献祭（终焉 main）
  e.player.buffs.push({ stat: 'haste', val: 40, turns: 99 });
  e.defendBonus = 1; // ap=2（保底 1 + 防御 +1）：s40+s03 双入队
  e.startTurn(); // apMax 5、ap 2
  const rS40 = e.queueMain('s40');
  t('K22a 终焉 s40 入队', rS40.ok, rS40.reason);
  const rS03 = e.queueMain('s03');
  t('K22b s03 可入队（入队期无终焉限制）', rS03.ok, rS03.reason);
  e.confirm();
  t('K22c 终焉扣光 AP：s03 被闸门跳过（跳过日志）', e.log.some((l) => l.text.includes('行动点不足，跳过 星尘咏叹')), e.log.map((l) => l.text).slice(0, 6).join('|'));
  t('K22d s40 献祭施放（终焉主技能）', e.log.some((l) => l.text.includes('灵魂献祭')));
  // F16：AP 上限 8 + 防御 +1 = 9（超上限）
  const e9 = eng();
  e9.player.buffs.push({ stat: 'haste', val: 70, turns: 99 });
  e9.defendBonus = 1;
  e9.startTurn();
  t('K22e AP8 + 防御 +1 = 9（超上限）', e9.apMax === 9, `apMax=${e9.apMax}`);
}
}
{
  // dot_enhance（s41）强化期间 dot 同源刷新（审查 F 项补测）
  const e = engCustom(['s04', 's41'], { mp: 300, maxMp: 300 });
  e.startTurn();
  e.queueMain('s04'); // 回合 1：挂 dot
  e.confirm();
  e.queueMain('s41'); // 回合 2：终焉虚空化身（独占）
  e.confirm();
  e.queueMain('s04'); // 回合 3：强化期间 dot 刷新（不叠加）
  e.confirm();
  t('K22 强化期间 dot 刷新不叠加（单条且 dotEnhanced 保持）', e.enemies[0].dots.filter((d) => d.stateName === '恶咒').length === 1 && e.player.dotEnhanced > 0, JSON.stringify(e.enemies[0].dots.map((d) => d.stateName)));
}

...
//   行动点（AP）：基础 1/回合，每 10% 急速 +1 上限（上限 8，2026-08-12）；主技能消耗 1
...
export class CombatEngine {
  ...
  startTurn() {
    ...
    //   不自动回满上限；上限 apMax = min(8, 1+急速/10) + 防御奖励（防御姿态下回合 +1，可超上限）；
  ...
  confirm() {
    ...
    for (const a of [...mains, ...instants]) {
      ...
      // AP 闸门（R1-A 高项）：终焉已扣光 AP 时，其后的非终焉主技能不施放——
      //   "扣光 AP 自然限制"必须有施放期执行（入队期校验看不到未来的清零，否则终焉队首白嫖后续技能）；
      //   回 AP 技（s01 净 0）豁免：ap=0 时仍可施放（回转——autoloop 平砍兜底依赖它）
      const selfRecover = (a.skill.effectRounds || []).some((e) => e.type === 'ap_recover');
      if (a.kind === 'main' && !a.skill.isFinale && !selfRecover && this.ap < 1) {
        // 封顶场景（apMax=1）回 AP 技先排会把 AP 窗口用掉 → 后续主技能被跳过——明确播报（正确操作：输出先排、回转后置）
        this._log(`行动点不足，跳过 ${a.skill.name}`, 'system');
        continue;
      }
      // 终检 R1-A 1.1：非终焉主技能**先扣后施放**（s01 等回 AP 技先扣再回 = 净 0 恒成立——
      //   封顶场景 grantAp 不丢 1，[s01, 输出] 顺序不再吞后续技能）；瞬发不占行动点
  ...
  // 终焉技能 = 消耗全部 AP（2026-08-12 不独占）：主技能施放时扣光（AP 闸门跳过后续非回 AP 主技能）；
  //   终焉瞬发不吃 AP（普通瞬发规则）
  queueMain(id) {

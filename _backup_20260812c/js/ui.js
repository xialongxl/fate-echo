...
const STAT_LABELS = {
  versa: '共鸣', haste: '急速', crit: '暴击', dmg_up_pct: '增伤', hp_pct: '生命', atk_pct: '攻击',
  // 2026-08-12 敌人技能从零重做：降攻/降急速/受疗降低中文名（chips 显示不露内部字段）
  atk_down_pct: '降攻', haste_down_pct: '降急速', heal_cut_pct: '受疗降低',
};
...
export class BattleUI {
  ...
  _renderPlayer() {
    ...
    panel.innerHTML = `
      ...
      <div class="unit-stats">
        ...
        <span>急速 <b>${esc(Math.max(0, p.statBonus('haste') - p.statBonus('haste_down_pct')).toFixed(1))}</b></span> <!-- 终检 F8：显示扣降急速后的有效值（与 apMax 口径一致） -->
  ...
  _renderResources() {
    ...
    this.els.resourceBar.innerHTML = `
      ...
      <span class="resource" title="每回合保底 1 行动点；基础攻击（魔力弹）/回 AP 技/防御姿态补充">保底 1 AP · 回转补充</span>`;
  }
  ...
  _renderSkills() {
    ...
    const mkBtn = (skill, keyLabel, isInstant) => {
      ...
      // 终焉大招提示（2026-08-12：终焉不独占；主技能终焉耗全 AP，瞬发终焉不吃 AP）
      if (skill.isFinale && !reason) btn.title = skill.isInstant ? '终焉技能（瞬发：不消耗行动点）' : '终焉技能（消耗全部行动点）';
      const apTag = (skill.isFinale && !skill.isInstant) ? '全 AP' : (skill.isInstant ? '' : '1 AP'); // R1-A：终焉瞬发不吃 AP，不标全 AP
      // 动态描述（末光照抄）：实际伤害/治疗值优先，倍率兜底
      const dyn = preview(skill);
      const mult = skill.dmgMult > 0 ? `×${skill.dmgMult}` : '';
      const meta = queued ? '✓ 已入队' : [dyn || mult, apTag].filter(Boolean).join(' · ');
      btn.innerHTML = `
...
export class OverworldUI {
  ...
  _renderGearDetail(gear, isEquipped) {
    ...
    statsEl.innerHTML = `
      <div class="stats-box">
        ...
        ${s.haste ? `<span><span class="label">急速</span><br><b>${s.haste.toFixed(1)}%</b></span>` : ''}
  ...
  // 收藏品独立背包（2026-08-11）：8 种收藏品图鉴式展示——已拥有彩色 + 效果说明，
  //   未拥有灰色；局内作用域（战斗掉落/商店购买，newGame 清空）
  renderCollections(game) {
    const owned = new Set(Array.isArray(game.player.collections) ? game.player.collections : []);
    this.renderPanel(`🏺 收藏品（${owned.size}/${COLLECTIONS_REF.length}）`, (panel) => {
      const hint = document.createElement('div');
      hint.className = 'panel-text';
      hint.textContent = '战斗掉落 / 商店购买获得；局内作用域（新游戏清空重收集）。';
      panel.appendChild(hint);
      const grid = document.createElement('div');
      grid.className = 'coll-grid';
      for (const c of COLLECTIONS_REF) {
        const card = document.createElement('div');
        card.className = 'coll-item' + (owned.has(c.id) ? ' owned' : ' locked');
        const name = document.createElement('div');
        name.className = 'coll-name';
        name.textContent = (owned.has(c.id) ? '✔ ' : '') + c.name;
        const desc = document.createElement('div');
        desc.className = 'coll-desc';
        desc.textContent = c.desc;
        const state = document.createElement('div');
        state.className = 'coll-state';
        state.textContent = owned.has(c.id) ? '已拥有' : '未拥有';
        card.appendChild(name);
  ...
  // 局外技能配置（2026-08-11）：从解锁技能中勾选上阵（常规 ≤5 / 终焉 ≤3），
  //   点击即时保存到 player.skillLoadout（null = 全部解锁）；未解锁灰色禁用；被动自动生效
  renderSkillConfig(game) {
    const p = game.player;
    const lv = Math.max(1, p.level);
    const loadout = p.skillLoadout;
    const selectedRegular = new Set(loadout && loadout.regular ? loadout.regular : []);
    const selectedFinale = new Set(loadout && loadout.finale ? loadout.finale : []);
    const onCount = (list, sel) => (loadout ? list.filter((s) => sel.has(s.id)).length : list.filter((s) => s.reqLv <= lv).length);
    const isFinaleData = (s) => /\[终焉\]/.test(s.desc || '');
    const regulars = SKILLS_DB_REF.filter((s) => !isFinaleData(s) && s.type !== 'passive').sort((a, b) => a.reqLv - b.reqLv);
    const finales = SKILLS_DB_REF.filter(isFinaleData).sort((a, b) => a.reqLv - b.reqLv);
    const regularOn = onCount(regulars, selectedRegular);
    const finaleOn = onCount(finales, selectedFinale);
    // 技能描述模板填充（data.js desc 为末光模板，2026-08-11 层数制简化后）：
    //   {dmg} 约伤害（无减伤近似）、{heal} 治疗、{shieldVal} 护盾；
    //   "持续{dur}层"（desc 已层数制）→ X 层；无使用限制（2026-08-11）；
    //   "每秒" → "每层"；终焉附加文案（2026-08-12：仅主技能终焉标"消耗全部行动点"，瞬发终焉不附加）
    const u = makeBattlePlayerRef(game).unit;
    const fillDesc = (s) => {
      const e0 = (s.effects || [])[0] || {};
      let d = (s.desc || '')
        .replace(/持续\{dur\}(?:秒|层)/g, `持续${Math.max(0, Math.round(e0.dur || 0))}层`)
        .replace(/每秒/g, '每层')
        .replace(/每(\d+)秒/g, '每$1层'); // "每2秒递增" → "每2层递增"（层数制）
      const vars = {
        dmgMult: s.dmgMult ? Math.round(s.dmgMult * 100) : 0,
        dmg: Math.round(u.atk * (s.dmgMult || 0)),
        healMult: e0.type === 'heal' ? Math.round((e0.val || 0) * 100) : 0,
        heal: e0.type === 'heal' ? Math.round(Math.max(u.atk, u.def) * (e0.val || 0)) : 0,
        hotPerTick: e0.type === 'hot' ? Math.round(u.maxHp * (e0.pct || 0)) : 0, // {hotPerTick}（s12/s19 每层恢复量）
        // mpRecover 已随 MP 删除（2026-08-11）：无技能使用该占位符
        shieldVal: e0.type === 'shield' ? Math.round(u.maxHp * (e0.hpPct || 0)) : 0,
        dps: e0.type === 'dot' ? Math.round((e0.dps || 0) * 100) : 0,
      };
      d = d.replace(/\{(\w+)\}/g, (m, k) => (k in vars ? vars[k] : m));
      if (isFinaleData(s) && s.type !== 'instant') d += '（终焉：消耗全部行动点）';
      return d;
    };
    this.renderPanel(`⚙ 技能配置（常规 ${regularOn}/5 · 终焉 ${finaleOn}/3）`, (panel) => {
      const hint = document.createElement('div');
      hint.className = 'panel-text';
      hint.textContent = '点击切换上阵（即时保存）；常规至少保留 1 个、上限 5 个；终焉上限 3 个（主技能终焉消耗全部行动点，瞬发终焉不吃 AP）；未解锁灰色；被动自动生效；新游戏重置为全部解锁。';
      panel.appendChild(hint);
      // 恢复全部解锁（null）：配置转换不可逆闭环（第 2 轮审查 C 项）
      if (loadout) {
        const reset = document.createElement('button');
        reset.className = 'opt-btn';
        reset.style.marginBottom = '8px';
        reset.textContent = '↩ 恢复全部解锁（默认）';
        reset.title = '取消所有配置，回到默认：全部解锁技能上阵';
        reset.addEventListener('click', () => this.handlers.onResetLoadout());
        panel.appendChild(reset);
      }
      const mkGroup = (title, list, sel, group) => {
        const head = document.createElement('div');
        head.className = 'panel-text atm-title';
        head.textContent = title;
        panel.appendChild(head);
        for (const s of list) {
          const row = document.createElement('div');
          const on = loadout ? sel.has(s.id) : s.reqLv <= lv;
          row.className = 'skill-config-item' + (on ? ' on' : '') + (s.reqLv > lv ? ' locked' : '');
          const info = document.createElement('div');
          info.style.flex = '1';
          // 摘要行（倍率——战斗数值速览；MP 与使用限制已删，2026-08-11）+ 完整描述行（模板填充）
          info.innerHTML = `<b>${esc(s.name)}</b> <span class="node-sub">Lv${s.reqLv}解锁${s.dmgMult > 0 ? ` · 倍率 ×${s.dmgMult}` : ''}</span><div class="node-sub" style="margin-top:2px;">${esc(fillDesc(s))}</div>`;
          row.appendChild(info);
          const btn = document.createElement('button');
          btn.className = 'opt-btn';
          btn.textContent = on ? '上阵 ✓' : '未上阵';
          btn.disabled = s.reqLv > lv;
          btn.addEventListener('click', () => this.handlers.onToggleSkill(s.id, group));
          row.appendChild(btn);
          panel.appendChild(row);
        }
      };
      mkGroup('── 常规技能（上限 5：主行动/瞬发）──', regulars, selectedRegular, 'regular');
      mkGroup('── 终焉技能（上限 3：主技能耗全 AP / 瞬发不吃 AP）──', finales, selectedFinale, 'finale');
      const note = document.createElement('div');
      note.className = 'panel-text';
      note.textContent = `被动技能（${SKILLS_DB_REF.filter((s) => s.type === 'passive').length} 个）自动生效，不参与配置。`;
      panel.appendChild(note);
    });
  }

  // AI 面板（永恒回想录 SmartAI 齐全度照抄：状态卡 + 自由开关 + 调试工具）
  //   ai：SmartAI 实例或 null（TF.js 不可用 → 提示降级）；settings：{aiLearning}
  //   handlers：{onToggleAI(enabled), onForceTrain, onClearAI, onExportAI}（main.js 注入）
  renderAIPanel(ai, settings, handlers = {}) {
    const stats = ai ? ai.getStats() : null;
    this.renderPanel('🧠 SmartAI', (panel) => {
      if (!ai) {
        const note = document.createElement('div');
        note.className = 'panel-text';
        note.textContent = 'TensorFlow.js 加载失败，SmartAI 不可用——敌人使用启发式 AI（游戏可正常游玩）。';
        panel.appendChild(note);
        const close = document.createElement('button');
        close.className = 'opt-btn';
        close.textContent = '关闭';
        // 走 onClose 复位 owMode（仅 closePanel 会软锁地图交互——审查 H1）
        close.addEventListener('click', () => { handlers.onClose && handlers.onClose(); });
        panel.appendChild(close);
        return;
      }
      // ---- 状态卡（永恒回想录 showAIStatus 框线内容 → 卡片） ----
      const card = document.createElement('div');
      card.className = 'stats-box';
      card.style.marginBottom = '12px';
      const readyText = stats.isModelReady ? '✅ 已就绪' : `⏳ 还需 ${stats.battlesNeeded} 条数据`;
      card.innerHTML = `
        <span><span class="label">战斗记录</span><br><b>${stats.totalBattles}</b></span>
        <span><span class="label">胜/负</span><br><b style="color:#2ecc71">${stats.wins}</b> / <b style="color:#ff6b6b">${stats.losses}</b></span>
        <span><span class="label">胜率</span><br><b>${stats.winRate}</b></span>
        <span><span class="label">训练数据</span><br><b>${stats.trainingDataCount} 条</b></span>
        <span><span class="label">模型状态</span><br><b>${readyText}</b></span>
        <span><span class="label">探索率 ε</span><br><b>${(stats.epsilon * 100).toFixed(1)}%</b></span>
        <span><span class="label">训练轮次</span><br><b>${stats.trainedBattles}</b></span>
        <span><span class="label">敌人分桶</span><br><b>${stats.enemyCount}</b></span>`;
      panel.appendChild(card);
      // ---- 自由开关（永恒回想录 setLearningEnabled 照抄；与战斗内 chk-ai 同步） ----
      const sw = document.createElement('label');
      sw.className = 'panel-text';
      ...
      chk.type = 'checkbox';
      chk.checked = !!settings.aiLearning;
      chk.addEventListener('change', () => {
        handlers.onToggleAI && handlers.onToggleAI(chk.checked);
        sw.querySelector('.ai-switch-text').textContent = chk.checked ? '敌人 AI 学习已开启' : '敌人 AI 学习已关闭（退回启发式）';
      });
      const label = document.createElement('span');
      label.className = 'ai-switch-text';
      label.textContent = settings.aiLearning ? '敌人 AI 学习已开启' : '敌人 AI 学习已关闭（退回启发式）';
      sw.appendChild(chk);
      sw.appendChild(label);
      panel.appendChild(sw);
      // ---- 调试工具（永恒回想录 forceTraining/clearAIData/exportTrainingData 照抄） ----
      const row = document.createElement('div');
      ...
      mkBtn('🎓 强制训练', '立即训练模型（不管数据量是否足够；训练日志输出到控制台）', () => handlers.onForceTrain && handlers.onForceTrain());
      mkBtn('🗑️ 清除 AI 数据', '删除所有战斗记录/训练数据/模型权重（需确认）', () => handlers.onClearAI && handlers.onClearAI());
      mkBtn('📦 导出训练数据', '将战斗记录/训练数据/模型元信息输出到控制台', () => handlers.onExportAI && handlers.onExportAI());
      panel.appendChild(row);
      const note = document.createElement('div');
      note.className = 'panel-text';
      note.style.marginTop = '10px';
      note.textContent = '战斗记录仅统计 AI 学习开启期间的战斗；SmartAI 全程日志输出到浏览器控制台（F12）：初始化/后端/训练 loss/战斗记录/行动反思。';
      panel.appendChild(note);
    });
  }
  ...
  renderGlossary(game, stats = null) {
    this.renderPanel('📖 术语表 · 帮助', (panel) => {
      ...
      const terms = [
        ['行动点 AP', '主技能消耗 1 点；每回合保底 1（回转收益留存）；每 10% 急速 +1 上限（默认 1，最高 8，防御姿态 +1 可超；敌人降急速 debuff 会压制上限）'],
        ['瞬发槽', '每回合 1 个瞬发槽位，不占行动点'],
        ['回合制', '持续效果为层数制（每回合 -1 层，减完结束）；技能无使用限制，可每回合重复施放'],
        ...
        ['宝珠', '5 种：生命/攻击/共鸣/暴击/终焉回响；同类最多镶嵌 3 个'],
        ['终焉精华', '分解终焉装备获得；精炼消耗'],
  ...
  _renderPlayer(game) {
    ...
    //   局外/战斗显示一致）；急速 = AP 上限加成（每 10% +1 行动点上限）
    const u = makeBattlePlayerRef(game).unit;
...
import { SKILLS_DB as SKILLS_DB_REF } from './data.js';
...
// 宝珠属性名（详情显示）
const ORB_NAMES_REF = { hp_pct: '生命加成', atk_pct: '攻击加成', versa: '共鸣', crit: '暴击', finale: '终焉回响' }; // finale 效果已随体系调整移除（2026-08-11）
// 词缀名（精炼详情显示）
const AFF_NAME_REF = { crit: '暴击率', haste: '急速', versa: '共鸣' }; // 急速 = AP 上限加成
// 终焉图鉴收集数

...
export const SMART_AI = {
  MODEL_VERSION: 2,       // 模型权重语义版本（审查 L2 + 终检 F1：2026-08-12 敌人技能池从零重做——旧 V1 权重在新技能语义下错位，bump 强制清空）
...
export class SmartAI {
  ...
  // 动作空间 = 非被动主行动技能 + 基础攻击（null）（终检 R1-A 7.1：排除瞬发——
  //   瞬发走瞬发槽决策，混入主行动会一回合 2 瞬发；与启发式 heuristicDecide 口径一致）
  _actionList(engine, enemyIndex) {
    return [...engine.enemySkillSets[enemyIndex].filter((s) => !s.isPassive && !s.isInstant), null];
  }

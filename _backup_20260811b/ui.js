...
export class BattleUI {
  ...
  _enemyChips(u) {
    ...
    for (const b of u.buffs) chips.push(`<span class="chip" title="增益">${esc(b.stat)} +${Number(b.val).toFixed(1)} ${fmtTurns(b.turns)}</span>`);
  ...
  _playerChips(p) {
    ...
    for (const b of p.buffs) {
      // 照抄末光 PlayerBuffs：buff 只显示限时状态——永久 buff（eq:/orb:/col: 装备/宝珠/
      //   收藏品属性已并入属性行 statBonus、passive: 被动常驻有"增伤"行）不重复展示；
      //   值 toFixed(1) 去浮点尾差
      if (b.turns === Infinity) continue;
      chips.push(`<span class="chip" title="增益">${esc(b.stat)} +${Number(b.val).toFixed(1)} ${fmtTurns(b.turns)}</span>`);
    }

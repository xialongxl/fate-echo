...
// 用例 H：全技能运行冒烟（53 技能逐一施放：构造→use→执行→冷却→回合结算）
...
const mkUnit = (o = {}) => new CombatUnit(Object.assign(
  { name: '源', hp: 500, maxHp: 500, mp: 200, maxMp: 200, atk: 100, int: 10, level: 10, critChance: 0 },
  o,
));
...
const ctx = {
  log: () => {},
  resetCooldowns: () => { resetCalled = true; },
  openDomain: (d) => { ctx.domain = d; },
};

// 单技能畸形 DB 构造器
const mkEntry = (over = {}) => Object.assign(
  { id: 't1', reqLv: 1, cd: 0, cost: 0, dmgMult: 0, type: 'gcd', priority: 1, effects: [] },
  over,
);
// 校验必须：不崩溃 + 报错
...
console.log('== B. 字段值畸形（Symbol/BigInt/对象/字符串/NaN/Infinity/负值）==');
{
  ...
  mustReject('B8 cd=字符串', mkEntry({ cd: 'abc' }));
  mustReject('B9 cd=NaN', mkEntry({ cd: NaN }));
  mustReject('B10 cd=负值', mkEntry({ cd: -5 }));
  mustReject('B11 cd=Symbol', mkEntry({ cd: Symbol('cd') }));
  mustReject('B12 cost=Infinity', mkEntry({ cost: Infinity }));
  mustReject('B13 cost=负值', mkEntry({ cost: -1 }));
  mustReject('B14 cost=字符串', mkEntry({ cost: 'x' }));
  mustReject('B15 cost=BigInt', mkEntry({ cost: 20n }));
...
console.log('== C. effects 畸形（非数组/条目类型/子字段）==');
{
  ...
  mustAccept('C29 cd_reset 无子字段合法', mkEntry({ effects: [{ type: 'cd_reset' }] }));
...
console.log('== D. Skill 构造对畸形 data 的容错 ==');
{
  ...
  err = safe(() => { s = new Skill({ cd: Symbol('cd') }); });
  t('D13 cd=Symbol → cdRounds=0', err === null && s.cdRounds === 0, err ? err.message : '');
  err = safe(() => { s = new Skill({ cd: '5000' }); });
  t('D14 cd=字符串 → 严格类型拒绝为 0', err === null && s.cdRounds === 0, err ? err.message : '');
  ...
  t('D20 data=null → 构造不崩溃（与无参同语义）', err === null && s.effectRounds.length === 0 && s.cdRounds === 0, err ? err.message : '');
...
console.log('== F. calcDamage 极端输入（结果必须有限）==');
{
  ...
  const f8 = calcDamage(src, mkTarget({ int: NaN }), 1.0, { noCrit: true });
  t('F8 target.int=NaN → 无减伤且有限', Number.isFinite(f8) && f8 > 0, `got=${f8}`);
  const f9 = calcDamage(src, mkTarget({ int: Infinity }), 1.0, { noCrit: true });
  t('F9 target.int=Infinity → 无减伤且有限', Number.isFinite(f9) && f9 > 0, `got=${f9}`);
  const f10 = calcDamage(src, mkTarget({ int: NaN, level: NaN }), 1.0, { noCrit: true });
  t('F10 target.int/level 均 NaN → 防 0/0，有限', Number.isFinite(f10) && f10 > 0, `got=${f10}`);
  const f11 = calcDamage(src, mkTarget({ level: NaN }), 1.0, { noCrit: true });
  t('F11 target.level=NaN → 有限', Number.isFinite(f11), `got=${f11}`);
  const f12 = calcDamage(src, mkTarget({ int: -10 }), 1.0, { noCrit: true });
  t('F12 target.int 负值 → 减伤夹取 0，有限', Number.isFinite(f12) && f12 > 0, `got=${f12}`);
...
console.log('== G. 合法数据回归（53 技能）==');
{
  ...
  t('G3 效果类型全集 = 14 种（含 mp_recover）', EFFECT_TYPES.length === 14 && DOMAIN_TYPES.length === 5);
}

// ---------- H. 全技能运行冒烟（53 技能逐一施放） ----------
console.log('== H. 全技能运行冒烟（53 技能：构造→use→执行→冷却→回合结算）==');
{
  ...
  for (const entry of SKILLS_DB) {
    ...
    for (const e of entry.effects || []) {
      switch (e.type) {
        ...
        case 'shield':
          ...
          break;
        case 'mp_recover_pct':
          t(`${tag} 回蓝生效`, src.mp > srcMp0, `mp=${src.mp}`);
          break;
        case 'vuln':
          t(`${tag} 易伤落位（×${e.val}）`, tg.vulnTurns > 0 && tg.vulnMult === e.val);
          break;
        case 'cd_reset':
          t(`${tag} 冷却重置回调触发`, resetCalled === true);
          break;
        case 'hp_sacrifice':
        ...
        case 'channel_immune':
          ...
          break;
        case 'cond_full_heal':
          t(`${tag} 条件回满（HP/MP 全恢复）`, src.hp === src.maxHp && src.mp === src.maxMp, `hp=${src.hp}, mp=${src.mp}`);
          break;
        case 'domain':
    ...
    // 冷却递减
    const expectCd = skill.cdRounds > 0 ? skill.cdRounds - 1 : 0;
    err = safe(() => skill.tick());
    if (err === null && skill.currentCd === expectCd) tickOk++;
    ...
    // 冷却递减断言单独记录（避免吞掉失败）
    if (skill.currentCd !== expectCd) {
      t(`${tag} tick 冷却递减`, false, `expect=${expectCd}, got=${skill.currentCd}`);
    }
  }
  t('H0 53 技能全部构造成功', constructOk === 53, `${constructOk}/53`);
  t('H1 53 技能首次 use 均成功', useOk === 53, `${useOk}/53`);
  t('H2 53 技能执行均不崩溃', execOk === 53, `${execOk}/53`);
  t('H3 53 技能冷却递减均正确', tickOk === 53, `${tickOk}/53`);
...
console.log('== I. 回合结算与状态防御（护盾 NaN / dot 缺 source / maxHp·maxMp 异常 / vuln val=0）==');
{
  ...
  const s7 = new CombatUnit({ name: 'i7', hp: 300, maxHp: NaN, atk: 100, int: 10, level: 10 });
  ...
  executeSkillEffects(ctx, new Skill({ id: 'i9', name: '无效易伤', reqLv: 1, type: 'debuff', cd: 0, cost: 0, dmgMult: 0, effects: [{ type: 'vuln', val: 0, dur: 10000 }] }), s9, t9);
  ...
  executeSkillEffects(ctx, new Skill({ id: 'i10', name: 'NaN 易伤', reqLv: 1, type: 'debuff', cd: 0, cost: 0, dmgMult: 0, effects: [{ type: 'vuln', val: NaN, dur: 10000 }] }), s10, t10);
  ...
  executeSkillEffects(ctx, new Skill({ id: 'i13', name: '零时易伤', reqLv: 1, type: 'debuff', cd: 0, cost: 0, dmgMult: 0, effects: [{ type: 'vuln', val: 1.2, dur: 0 }] }), s13, t13);
  ...
  executeSkillEffects(ctx, new Skill({ id: 'i18', name: '零时dot', reqLv: 1, type: 'dot', cd: 0, cost: 0, dmgMult: 0, effects: [{ type: 'dot', dps: 5, dur: 0 }] }), s18, t18);
  ...
  executeSkillEffects(ctx, new Skill({ id: 'i19', name: '零时hot', reqLv: 1, type: 'buff', cd: 0, cost: 0, dmgMult: 0, effects: [{ type: 'hot', pct: 0.05, dur: 0 }] }), s19, s19);
  ...
  const s22 = new CombatUnit({ name: 'i22', hp: 400, maxHp: NaN, atk: 100, int: 10, level: 10 });
  const t22 = mkTarget({ hp: 9999 });
  executeSkillEffects(ctx, new Skill({ id: 'i22', name: 'NaN献祭', reqLv: 1, type: 'gcd', cd: 0, cost: 0, dmgMult: 0, effects: [{ type: 'hp_sacrifice', costPct: 0.5, dmgMult: 1 }] }), s22, t22);
  ...
  const s23 = new CombatUnit({ name: 'i23', hp: NaN, maxHp: 500, atk: 100, int: 10, level: 10 });
  const t23 = mkTarget({ hp: 9999 });
  executeSkillEffects(ctx, new Skill({ id: 'i23', name: 'NaN生命献祭', reqLv: 1, type: 'gcd', cd: 0, cost: 0, dmgMult: 0, effects: [{ type: 'hp_sacrifice', costPct: 0.5, dmgMult: 20 }] }), s23, t23);

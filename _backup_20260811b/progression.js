...
//   每级成长：HP +35 / 攻击 +8 / 防御 +5 / 暴击 +1%（初值 HP100/攻10/防0/
//     暴击5%/急速0/共鸣0；int 不成长，参与减伤公式）
...
// 玩家基础属性（Level 1，§9）
export const PLAYER_BASE = { hp: 100, mp: 100, atk: 10, def: 0, int: 10, critChance: 0.05 };
...
export function makeBattlePlayer(state) {
  ...
  // 收藏品加成（局内，2026-08-11）：hp/atk 乘算、crit/versa 加算（与终焉图鉴同级）
  const col = collectionBonus(state.player);
  if (col.hp_pct > 0) {
    unit.maxHp = Math.floor(unit.maxHp * (1 + col.hp_pct / 100));
    unit.hp = Math.min(unit.maxHp, unit.hp);
  }
  if (col.atk_pct > 0) unit.atk = Math.floor(unit.atk * (1 + col.atk_pct / 100));
  ...
  // 终焉图鉴集齐：最大生命 +10% / 攻击 +10%（末光 finaleCollection）
  if (finaleCollectionComplete(state.player)) {
    unit.maxHp = Math.floor(unit.maxHp * 1.1);
    unit.hp = Math.min(unit.maxHp, unit.hp);
    unit.atk = Math.floor(unit.atk * 1.1);
  }
  return { unit, skills: unlockedSkillData(lv) };
}

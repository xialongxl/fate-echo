...
const mkEnemy = (o = {}) => new CombatUnit(Object.assign(
  { name: '靶子', hp: 300, maxHp: 300, mp: 100, maxMp: 100, atk: 12, int: 10, level: 5, critChance: 0 },
  o,
));
...
console.log('== A. 保命链 ==');
{
  // 绝境：HP<30% → s45 回满 + 常规输出（不排治疗，防条件失效）
  const e = eng({ hp: 50 });
  autoLoopTurn(e);
  t('A1 HP 25% 时自动 命运轮转（回满 → 受击后 200-9）', e.player.hp === 200 - PLAYER_HIT && e.log.some((l) => l.text.includes('命运逆转')), `hp=${e.player.hp}`);
...
}
{
  ...
  const e = eng({ mp: 30 });
  autoLoopTurn(e);
  t('A3 MP 30% 时自动 黑暗契约（恢复 8 法力）', e.log.some((l) => l.text.includes('恢复 8 法力')), e.log.map((l) => l.text).join('|'));
}
{
  // 满血满蓝无盾 → 不用任何瞬发
  const e = eng();
  autoLoopTurn(e);
  t('A4 状态健康时瞬发槽空置（无恢复/护盾日志）', !e.log.some((l) => l.text.includes('恢复') || l.text.includes('护盾')));
}

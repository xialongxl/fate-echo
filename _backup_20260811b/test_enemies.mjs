...
import { createEnemy, createRandomEnemy, ENEMIES_DB, RANDOM_NAMES, bossTitle, allEnemies } from '../js/enemies.js';
...
console.log('== D. 模板保留 ==');
{
  ...
  t('D5 非法 tier 容错（回退 normal）', bad.meta.tier === 'normal' && bad.unit.maxMp === 100);
  ...
  t('D7 Boss 法力 300（领域支撑）', b.unit.maxMp === 300);
  ...
  t('D9 随机精英继承吸血技能（挑中虚空猎手）', hunter.skills.some((s) => s.id === 'e10_vamp'), hunter.skills.map((s) => s.id).join());
}

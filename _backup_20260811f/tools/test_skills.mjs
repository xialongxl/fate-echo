...
console.log('== 类型与条件 ==');
t('主技能(main) 20 个', skills.filter((s) => s.isMain).length === 20);
t('瞬发(instant) 9 个', skills.filter((s) => s.isInstant).length === 9);
...
t('效果类型全集 12 种（含 ap_recover 回行动点；mp_recover/重置类已删）', EFFECT_TYPES.length === 12 && EFFECT_TYPES.includes('ap_recover') && !EFFECT_TYPES.includes('mp_recover'));

...
// ---------- B. Skill 状态机 ----------
console.log('== B. Skill.use / conditionMet ==');
const s03 = new Skill(SKILLS_DB.find((s) => s.id === 's03'));
t('s03 连续 use 恒 true（无使用限制）', s03.use() && s03.use());
const s07 = new Skill(SKILLS_DB.find((s) => s.id === 's07'));
t('s07 连续 use 恒 true（无使用限制）', s07.use() && s07.use());
const s01 = new Skill(SKILLS_DB.find((s) => s.id === 's01'));
t('s01 连续 use 恒 true（无使用限制）', s01.use() && s01.use());
...
t('s45 canUse 组合（条件）', s45.canUse({ hpPct: () => 0.2 }) && !s45.canUse({ hpPct: () => 0.5 }));
...
console.log('== E. executeSkillEffects 单次结算 / 空效果 / 容错 ==');
{
  ...
  const fake1 = new Skill({ id: 'x01', name: '测试打击', reqLv: 1, type: 'main', dmgMult: 1.0, effects: [] });
  ...
  const fake2 = new Skill({ id: 'x02', name: '未知效果', reqLv: 1, type: 'main', dmgMult: 0, effects: [
    { type: 'zzz_unknown' },
  ...
  t('s38 回 AP 效果 durRounds=0（无 dur）', s38.effectRounds[0].durRounds === 0);
...
//   dur 毫秒→层数、desc 文案（秒→层/每层、终焉标注）——对照排除这些字段
// 2026-08-11 删 MP/无使用限制/AP 回转改造的技能（与源有意不同，effects 对照豁免）
const REWORKED = new Set(['s01', 's07', 's30', 's34', 's38', 's46']);
const FIELDS = ['id', 'name', 'reqLv', 'dmgMult', 'priority', 'conditionMaxHPPct']; // cost/type 已删（有意改造：cost 删、type 改 main/instant）

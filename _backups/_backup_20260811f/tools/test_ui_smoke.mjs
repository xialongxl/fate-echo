...
const makeEl = (tag) => {
  const el = {
    ...
    // 简易 querySelector（支持 .class 多类 / [attr] / [attr="v"]；单数版委托）
...
console.log('== A. 启动与区域地图 ==');
{
  ...
  t('A4b 局外属性面板 = 末光 stats-grid UI 形式（stat-item 卡片六项）', (() => {
    const h = el('ow-player')._innerHTML;
    return h.includes('stat-item') && h.includes('攻击力') && h.includes('防御力') && h.includes('急速') && h.includes('暴击率') && h.includes('共鸣') && !h.includes('eq-slot');
...
console.log('== J. 失败与读档 ==');
{
  // 复位：newGame 重置玩家为 Lv1（不 continueGame——AUTO 槽是 H 组高等级存档，读回会打赢 Boss）
  fe().newGame();
  // 构造必败：玩家 1 血进 Boss 战（s45 命运轮转为终焉——自动模式不用 → Lv1 输出打不动 Boss → 必死）
  fe().game.player.hp = 1;
  fe().game.player.mp = 0;
  fe().area.rng = () => 0.5; // area.rng 固定，保证敌人/Boss 确定性
  ...
  while (fe().view === 'battle' && guard++ < 2000) await sleep(5);
  fireKey('Tab');
  t('J1 失败 → gameover 视图', fe().gameover === true, `gameover=${fe().gameover} view=${fe().view} hp=${fe().game.player.hp}`);
...
console.log('== O. 面板弹窗 header 关闭（X = 放弃节点，防取消重刷） ==');
{
  ...
  // AI 面板：按钮存在 + 打开（TF.js 不可用时显示降级说明；面板结构渲染不崩）
  t('O7 AI 按钮存在（🧠 SmartAI 面板入口）', !!el('btn-ai'));
  fireClick(el('btn-ai'));
  await sleep(10);
  t('O8 AI 面板打开（面板弹窗 + 标题）', el('panel-overlay').style.display === 'flex' && el('panel-title').textContent.includes('SmartAI'), el('panel-title').textContent);
  // O8b 状态卡字段（审查 B：断言真实渲染而非仅标题）
  const aiText = owPanelText();
  t('O8b 状态卡字段齐全（战斗记录/训练数据/模型状态/探索率）', aiText.includes('战斗记录') && aiText.includes('训练数据') && aiText.includes('模型状态') && aiText.includes('探索率'), aiText.slice(0, 120));
  // O8c 面板开关与 chk-ai 双向同步（审查 B：面板 handler 写回 chk-ai）
  const allNodes = (n) => [n, ...[...(n.children || [])].flatMap(allNodes)];
  const chk = allNodes(el('panel-body')).find((n) => n.type === 'checkbox'); // 桩无 tagName，用 type 匹配
  const globalChk = el('chk-ai');
  t('O8c 面板含学习开关且与 chk-ai 初始一致', !!chk && !!globalChk && chk.checked === globalChk.checked, `panel=${chk && chk.checked} global=${globalChk && globalChk.checked}`);
  chk.checked = !chk.checked;
  for (const fn of chk.listeners.change || []) fn({ target: chk });
  t('O8d 面板开关切换同步 chk-ai（双向联动）', globalChk.checked === chk.checked, `panel=${chk.checked} global=${globalChk.checked}`);
  chk.checked = !chk.checked;
  for (const fn of chk.listeners.change || []) fn({ target: chk });
  // O8h 调试按钮点击（R2-B 3d 盲区：强制训练/清除/导出从未被点击——桩环境空回放安全）
  t('O8h 调试按钮齐全（强制训练/清除/导出）', !!findBtn(el('panel-body'), '强制训练') && !!findBtn(el('panel-body'), '清除 AI 数据') && !!findBtn(el('panel-body'), '导出训练数据'));
  const btnForce = findBtn(el('panel-body'), '强制训练');
  fireClick(btnForce);
  await sleep(30); // forceTraining 异步（空回放返回空数组，不崩）
  const btnExport = findBtn(el('panel-body'), '导出训练数据');
  fireClick(btnExport);
  t('O8i 强制训练/导出点击不崩（空回放安全）', true);
  fireClick(el('panel-close'));
  // O8e 降级分支（审查 H1：ai=null 时面板渲染 + 关闭按钮走 onClose 复位，防地图交互软锁）
  let aiClosed = false;
  fe().renderAIPanel(null, { aiLearning: true }, { onClose: () => { aiClosed = true; } });
  t('O8e 降级面板打开（TF.js 不可用说明）', el('panel-overlay').style.display === 'flex' && owPanelText().includes('TensorFlow.js'), owPanelText().slice(0, 60));
  const dbgClose = findBtn(el('panel-body'), '关闭');
  t('O8f 降级面板含关闭按钮', !!dbgClose);
  fireClick(dbgClose);
  t('O8g 降级关闭走 onClose 回调（不复位 owMode 会软锁）', aiClosed === true);
  fireClick(el('panel-close'));
}
...
console.log('== R. 背包详情 + 属性显示（对齐末光） ==');
{
  ...
  t('R8 属性行显示急速/共鸣数值（属性全称"攻击/防御"）', playerHtml.includes('急速') && playerHtml.includes('共鸣') && playerHtml.includes('攻击') && playerHtml.includes('防御'), playerHtml.slice(0, 200));
...
console.log('== T. 局外技能配置（即时保存 + 战斗生效 + 主技能守卫） ==');
{
  ...
  const countMain = (t2) => { const m = (t2 || '').match(/常规 (\d+)/); return m ? +m[1] : -1; };
  ...
  t('T3 常规技能行有上阵按钮（点击切换）', !!btn1 && btn1.listeners && btn1.listeners.click && btn1.listeners.click.length > 0);
  fireClick(btn1);
  const c2 = countMain(el('panel-title').textContent);
  t('T4 取消后计数 -1 且即时保存（skillLoadout.regular）', c2 === c1 - 1 && fe().game.player.skillLoadout && fe().game.player.skillLoadout.regular.length === c2, `${c1}→${c2}`);
  ...
  // 守卫：最后 1 个常规技能拒绝取消（防战斗无行动卡死）
  fe().newGame();
  fe().setRng(() => 0.5);
  fe().game.player.level = 5;
  fireClick(el('btn-skills'));
  const c3 = countMain(el('panel-title').textContent);
  for (let i = 0; i < c3 - 1; i++) { // 取消到只剩 1 个常规技能
    const onRow = firstOn();
    if (!onRow) break;
    fireClick(onRow.children[1]);
  }
  const c4 = countMain(el('panel-title').textContent);
  t('T7 取消到只剩 1 个常规技能', c4 === 1, `c4=${c4}`);
  fireClick(firstOn().children[1]); // 再取消 → 守卫拒绝
  t('T8 最后 1 个常规技能拒绝取消（计数仍 1）', countMain(el('panel-title').textContent) === 1 && fe().game.player.skillLoadout.regular.length === 1, el('panel-title').textContent);
  // 恢复全部解锁（配置不可逆闭环）
  ...
  // 技能描述模板填充：desc 占位符全部替换（含 {dps}），摘要行保留（倍率）
  ...
  t('T13 摘要行保留（倍率 ×；无使用限制）', descText(el('panel-body')).includes('倍率 ×1'), descText(el('panel-body')).slice(0, 200));
  fireClick(el('panel-close'));
}

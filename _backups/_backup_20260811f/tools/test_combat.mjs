...
{
  // 2026-08-11 无使用限制 + 终焉独占：s41 虚空化身（终焉）独占回合——s04 无法同回合入队
  const e = engCustom(['s04', 's41'], { mp: 200, maxMp: 200 });
  e.player.buffs.push({ stat: 'haste', val: 15, turns: 99 });
  e.startTurn();
  t('K9a 终焉 s41 入队（队列空）', e.queueMain('s41').ok);
  t('K9b 终焉入队后常规 s04 被拒（独占全 AP）', !e.queueMain('s04').ok && e.queueMain('s04').reason === '终焉技能已消耗全部行动点');
  e.confirm();
  ...
  e.queueMain('s04');
  e.confirm();
  t('K10 dot 4 倍（强化期间：立即伤 + 双结算 2×30）', e.enemies[0].hp === 300 - expectDmg(0.5) - 2 * expectDmg(1.6), `hp=${e.enemies[0].hp}`);
}
{
  ...
  e.queueMain('s04');
  e.confirm();
  e.queueInstant('s38'); // 终焉瞬发入队（瞬发槽被终焉独占）
  t('K11 终焉瞬发入队后常规 main 被拒（s04 独占全 AP）', !e.queueMain('s04').ok && e.queueMain('s04').reason === '终焉技能已消耗全部行动点', e.queueMain('s04').reason);
  e.confirm();
  t('K12 s38 急速 +50 ×2 层（回合 2 结束 → 剩 1）', e.player.buffs.some((b) => b.stat === 'haste' && b.val === 50 && b.turns === 1));
}
{
  // 终焉独占完整：常规先入队 → 终焉被拒（需先清空队列）
  const e = engCustom(['s03', 's39'], { mp: 300, maxMp: 300 });
  e.startTurn();
  e.queueMain('s03');
  t('K12d 常规入队后终焉 s39 被拒（消耗全部行动点）', !e.queueMain('s39').ok && e.queueMain('s39').reason === '终焉技能消耗全部行动点（请先清空队列）', e.queueMain('s39').reason);
  e.clearPending();
  t('K12e 清空后终焉可入队', e.queueMain('s39').ok);
  e.confirm();
...
{
  // 防御姿态 +1AP × 终焉独占（审查 F 项补测）
  const e = engCustom(['s03', 's01', 's39'], { mp: 300, maxMp: 300 });
  e.startTurn();
  e.defend(); // 回合 2：apMax = 2
  t('K20 防御 +1AP：常规占满（2 main）后终焉仍被拒', e.queueMain('s03').ok && e.queueMain('s01').ok && !e.queueMain('s39').ok, `apMax=${e.apMax}`);
  e.clearPending();
  t('K21 清空后终焉可入队（AP 上限不构成额外限制）', e.queueMain('s39').ok);
  e.confirm();
  t('K21b 终焉回合正常推进', e.turn === 3);
}
{

...
// 对敌：dot/易伤/反斩杀/比例伤害/降攻/降急速/受疗降低；hp_sacrifice 混合（source 扣血、target 出伤，handler 内自行处理）
...
export function calcDamage(source, target, dmgMult, opts = {}) {
  ...
  // 降攻修正（atk_down debuff，2026-08-12 敌人技能从零重做新增）：上限 80%（不归零）
  const atkDown = typeof source.statBonus === 'function' ? Math.min(80, Math.max(0, source.statBonus('atk_down_pct'))) : 0; // Number.isFinite(函数) 恒 false——用 typeof 守卫
  // R2-A 终检：先乘后除避免 (1-x/100) 浮点下溢（atkDown 80% 时 19 而非 20）
  let dmg = Math.floor(source.atk * (100 - atkDown) * dmgMult / 100);
  // 暴击（暴击率上限 100%，×1.5）
...
// 共享的 stat buff 刷新（buff / atk_down / haste_down / heal_cut 共用，2026-08-12）：
//   同类（同 stat）条目 val/层数取 max（短施放不缩短已有）；永久 buff（turns=Infinity）不受影响
const STAT_LABEL_REF = { atk_down_pct: '降攻', haste_down_pct: '降急速', heal_cut_pct: '受疗降低' }; // R2-B：日志不暴露内部 stat 名
function applyStatBuff(target, e, ctx) {
  ...
  ctx.log(`${target.name} 获得 ${STAT_LABEL_REF[e.stat] || e.stat} +${e.val}（${e.durRounds} 层）`);
}
...
export const EFFECT_HANDLERS = {
  ...
  buff(ctx, target, source, e) {
    applyStatBuff(target, e, ctx);
  },

  // 降攻/降急速/受疗降低（2026-08-12 敌人技能从零重做新增）：共享 applyStatBuff 刷新
  //   （对敌路由：SELF_EFFECTS 不含这些类型 → executeSkillEffects 自动路由到 target；
  //    stat 名由效果类型映射——效果数据只写 val/dur）
  atk_down(ctx, target, source, e) { if (e.durRounds <= 0) return; applyStatBuff(target, { ...e, stat: 'atk_down_pct' }, ctx); },
  haste_down(ctx, target, source, e) { if (e.durRounds <= 0) return; applyStatBuff(target, { ...e, stat: 'haste_down_pct' }, ctx); },
  heal_cut(ctx, target, source, e) { if (e.durRounds <= 0) return; applyStatBuff(target, { ...e, stat: 'heal_cut_pct' }, ctx); },

  // 反斩杀（lost_hp_dmg，深渊领主招牌）：伤害 = 目标已损失生命 × pct
  //   noCrit 稳定（比例伤害不吃暴击）；吃减伤/乘区；目标满血时 0 伤害（迫使玩家保持血线）
  lost_hp_dmg(ctx, target, source, e) {
    const lost = Math.max(0, (Number.isFinite(target.maxHp) ? target.maxHp : 0) - target.hp);
    if (lost <= 0 || target.hp <= 0) {
      ctx.log(`${target.name} ${target.hp <= 0 ? '生命已耗尽' : '生命完好'}，${source.name} 的反斩杀未造成伤害`);
      return;
    }
    const dmgMult = lost * (e.pct || 0) / (source.atk || 1);
    const dmg = calcDamage(source, target, dmgMult, { noCrit: true, rng: ctx && ctx.rng });
    const actual = target.takeDamage(dmg);
    ctx.log(`${target.name} 受 ${source.name} 的反斩杀：-${actual}`);
  },

  // 最大生命比例伤害（maxhp_dmg）：伤害 = 目标最大生命 × pct（noCrit 稳定，吃减伤）
  maxhp_dmg(ctx, target, source, e) {
    const pct = Math.min(0.5, Math.max(0, e.pct || 0)); // 上限 50%（防畸形配置）
    const dmgMult = (Number.isFinite(target.maxHp) ? target.maxHp : 0) * pct / (source.atk || 1);
    const dmg = calcDamage(source, target, dmgMult, { noCrit: true, rng: ctx && ctx.rng });
    const actual = target.takeDamage(dmg);
    ctx.log(`${source.name} 侵蚀 ${target.name} → 造成 ${actual} 点伤害`);
  },

  // 连击（multi_hit）：dmgMult 结算 hits 次（每次独立暴击/结算；日志合并）
  multi_hit(ctx, target, source, e, skill) {
    const hits = Math.max(1, Math.min(10, Math.round(e.hits || 1)));
    let total = 0;
    for (let i = 0; i < hits; i++) {
      const dmg = calcDamage(source, target, e.dmgMult || 0, { rng: ctx && ctx.rng });
      total += target.takeDamage(dmg);
    }
    ctx.log(`${source.name} 使用 ${skill.name} → 连击 ${hits} 次共 ${total} 点伤害`);
  },

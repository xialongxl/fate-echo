...
export const ORBS = [
  ...
  { id: 'orb_finale', name: '终焉回响', stat: 'finale', val: 20, desc: '终焉回响（2026-08-11 体系调整后效果已移除）' },
];
...
export function calcOrbStatAndScore(orb) {
  // finale 宝珠效果已删（2026-08-11 体系调整）——不计评分（R1-A：死属性虚高误导自动穿戴比较）
  if (orb.stat === 'finale') return { score: 0 };
...
export function calcOrbBonus(player) {
  const orbBonus = { hp_pct: 0, atk_pct: 0, versa: 0, crit: 0, finale: 0 };
  const counts = {};
  for (const slot of SLOTS) {
    const g = player.equipment && player.equipment[slot];

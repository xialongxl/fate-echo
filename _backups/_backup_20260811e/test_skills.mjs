...
console.log('== 回合制换算（2026-08-11 去 CD：仅回蓝技保留冷却） ==');
const skills = SKILLS_DB.map((d) => new Skill(d));
t('53 个 Skill 全部可实例化', skills.length === 53);
const expect = { s01: 0, s03: 0, s06: 0, s09: 0, s14: 0, s32: 0, s39: 0, s38: 0, s40: 0, s47: 0, s07: 8, s34: 24 };
...
const s07 = skills.find((s) => s.id === 's07'); // 回蓝技保留 CD 8 回合
t('s07 use 置冷却 8 且不可用', s07.use() && s07.currentCd === 8 && !s07.ready);
t('冷却中 use 返回 false', !s07.use());
s07.tick();
t('tick 后冷却递减（8 → 7）', s07.currentCd === 7);
for (let i = 0; i < 7; i++) s07.tick();
t('冷却归零可用', s07.currentCd === 0 && s07.ready);
...
t('效果类型全集 14 种（含 mp_recover 固定回蓝）', EFFECT_TYPES.length === 14);

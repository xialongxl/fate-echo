...
const s07 = new Skill(SKILLS_DB.find((s) => s.id === 's07')); // 回蓝技保留 CD 8 回合
t('s07 冷却 8 回合', s07.cdRounds === 8);
t('s07 首次 use → true（置冷却 8）', s07.use() === true && s07.currentCd === 8);
t('s07 冷却中连续 use → false', s07.use() === false && s07.currentCd === 8);
t('s07 冷却中 use 不改变 currentCd', s07.currentCd === 8);
s07.tick();
t('s07 tick 一次 8 → 7', s07.currentCd === 7);
for (let i = 0; i < 7; i++) s07.tick();
t('s07 tick×8 → 0 可用', s07.currentCd === 0 && s07.ready);
s07.tick();
...
const FIELDS = ['id', 'name', 'reqLv', 'type', 'cost', 'dmgMult', 'priority', 'conditionMaxHPPct'];

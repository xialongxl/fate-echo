...
//   冷却回合化 / 持续效果（dot/hot/buff/shield/vuln）回合结束结算
...
export const AP_MAX = 3;                  // 行动点上限
export const AP_HASTE_STEP = 10;          // 每 10% 急速 +1 AP 上限
export const MP_REGEN_PCT = 0.01;         // 玩家每回合自然回蓝 1% 最大法力
...
export class CombatEngine {
  ...
  constructor({ player, enemies = [], rng = Math.random, ai = null } = {}) {
    ...
    // 效果处理器上下文（effects.js 依赖：log/resetCooldowns/openDomain/rng）
    this._ctx = {
      rng: this.rng,
      log: (text) => this._log(text, 'info'),
      resetCooldowns: (unit) => this._resetCooldowns(unit),
  ...
  startTurn() {
    ...
    // 行动点：基础 1 + floor(急速/10)，急速部分上限 3；防御姿态下回合 +1（独立叠加，可超上限）
    // （§2.2"上限 3"指急速加成上限；防御奖励是独立的 +1，锁定行为见 test B13）
    const haste = this.player.statBonus('haste');
    this.apMax = Math.min(AP_MAX, AP_BASE + Math.floor(haste / AP_HASTE_STEP)) + this.defendBonus;
    this.ap = this.apMax;
    this.defendBonus = 0;

    // 玩家自然回蓝 1% 最大法力（NEW_GAME.md §2.4）
    const before = this.player.mp;
    this.player.restoreMp(this.player.maxMp * MP_REGEN_PCT);
    if (this.player.mp > before) {
      this._log(`回合 ${this.turn} 开始：自然回蓝 +${this.player.mp - before}`, 'info');
    }
  ...
  // 玩家行动编排
  // ============================================================
  // 主技能入队（消耗行动点；NEW_GAME.md §3.2：除 ogcd/passive 外的类型（gcd/buff/dot/debuff/domain）均为主行动）
  // 终焉技能 = 消耗全部 AP（2026-08-11）：独占本回合行动——入队前队列须为空、AP≥1；
  //   入队后其他技能不可再入队（替代 CD 成为大招限制）
  queueMain(id) {
    ...
    if (c.skill.isFinale) {
      if (this.pending.length > 0) return { ok: false, reason: '终焉技能消耗全部行动点（请先清空队列）' };
      if (this.ap < 1) return { ok: false, reason: '行动点不足' };
    } else {
      if (this.pending.some((p) => p.skill.isFinale)) return { ok: false, reason: '终焉技能已消耗全部行动点' };
      if (this.pending.filter((p) => p.kind === 'main').length >= this.ap) return { ok: false, reason: '行动点不足' };
    }
  ...
  canQueueMain(id) {
    ...
    if (c.skill.isFinale) {
      if (this.pending.length > 0) return '终焉技能消耗全部行动点（请先清空队列）';
      if (this.ap < 1) return '行动点不足';
    } else {
      if (this.pending.some((p) => p.skill.isFinale)) return '终焉技能已消耗全部行动点';
      if (this.pending.filter((p) => p.kind === 'main').length >= this.ap) return '行动点不足';
    }
    return '';
  }
  canQueueInstant(id) {
  ...
  // 施放核心
  // ============================================================
  // 可施放基础校验（玩家侧：阶段 + 冷却 + 血量条件 + 法力 + 献祭生命门槛）；返回 {ok, reason, skill?}
  _validateQueuable(id) {
    ...
    if (!skill.canUse(this.player)) return { ok: false, reason: skill.ready ? '条件未满足' : '冷却中' };
    if (skill.cost > this.player.mp) return { ok: false, reason: '法力不足' };
  ...
  // 施放一个技能（扣除法力 → 置冷却 → 执行效果）。player 侧在 confirm 内调用，敌人侧在敌人阶段调用。
  _cast(unit, skill) {
    const side = unit === this.player ? 'player' : 'enemy';
    if (!skill.canUse(unit)) { this._log(`${unit.name} 的 ${skill.name} 无法施放（${skill.ready ? '条件未满足' : '冷却中'}）`, 'system'); return; }
    if (skill.cost > unit.mp) { this._log(`${unit.name} 法力不足，${skill.name} 施放失败`, 'system'); return; }
    unit.mp = Math.max(0, unit.mp - skill.cost);
    // 终焉回响（宝珠 orb_finale）：终焉技能冷却缩减 %（末光照抄语义：缩短终焉技能冷却 20%）
    //   2026-08-11 去 CD 后终焉无冷却（cdRounds=0），回响仅对仍保留 CD 的终焉生效（当前无），
    //   无 cdRounds 时走 use()（cd=0 → 不置冷却）
    if (unit.finaleCdReduction > 0 && skill.isFinale && skill.cdRounds > 0) {
      skill.currentCd = Math.max(1, Math.floor(skill.cdRounds * (1 - unit.finaleCdReduction / 100)));
    } else {
      skill.use();
    }
    if (skill.cost > 0) this._log(`${unit.name} 消耗 ${skill.cost} 法力施放 ${skill.name}`, 'info');
  ...
  _enemyPhase() {
    ...
    for (let i = 0; i < this.enemies.length; i++) {
      ...
      const actionIsFinale = !!(action && action.isFinale); // 终焉 = 消耗全部行动（不再放瞬发）
      if (action) this._cast(enemy, action);
      else this._basicAttack(enemy, this.player);
      // 精英/Boss：主行动后 0~1 次瞬发（NEW_GAME.md §2.1）；终焉主行动后跳过（已消耗全部行动）
      let instant = null;
      if (!actionIsFinale && meta && meta.tier !== 'normal' && this.player.alive && this.phase !== 'ended') {
        instant = this._decideInstant(i, enemy, skills);
        if (instant) this._cast(enemy, instant);
      }
      // AI 学习钩子：该敌人一步（主行动+瞬发）完成；战斗已结束（击杀）时跳过——
      // 终局奖励由 onBattleEnd 记录，避免击杀动作信号错位
      if (this.ai && this.ai.onEnemyStep && this.phase !== 'ended') this.ai.onEnemyStep(this, i, action || instant);
  ...
  // ============================================================
  // 回合结束：领域伤害 → dot/hot 结算 → 状态递减 → CD 递减 → 领域到期 → 胜负
  // ============================================================
  _endTurn() {
    this._tickDomains();
    this._tickDotsHots();
    for (const u of [this.player, ...this.enemies]) u.tickStatus();
    for (const s of this._allSkills()) s.tick();
  ...
  // ---- 冷却重置（cd_reset 效果）----
  // 末光照抄：时间回溯（s38）只重置非终焉技能（终焉技能冷却独立）
  _resetCooldowns(unit) {
    const skills = unit === this.player ? this.playerSkills : this.enemySkillSets[this.enemies.indexOf(unit)] || [];
    for (const s of skills) {
      if (s.isFinale) continue;
      s.currentCd = 0;
    }
  }

...
{
  // 去 CD 后仅回蓝技 s07 保留冷却（8 回合）：冷却拒绝/递减语义用 s07 验证
  const e = eng();
  e.startTurn();
  e.queueInstant('s07');
  e.confirm(); // s07 cd 8 → 回合结束递减为 7
  t('C6 冷却中再次入队拒绝（s07 cd8 → 剩 7）', !e.queueInstant('s07').ok && e.queueInstant('s07').reason === '冷却中');
  e.queueMain('s01');
  e.confirm();
  t('C7 冷却逐回合递减（7 → 6）', e.playerSkills.find((s) => s.id === 's07').currentCd === 6, `cd=${e.playerSkills.find((s) => s.id === 's07').currentCd}`);
}
{
...
}
{
  ...
  const e = engCustom(['s02', 's04', 's13'], { mp: 300, maxMp: 300 }, { hp: 1000, maxHp: 1000, atk: 0 });
  e.player.buffs.push({ stat: 'haste', val: 15, turns: 99 });
  e.startTurn();
  e.queueMain('s02');
  e.queueMain('s04');

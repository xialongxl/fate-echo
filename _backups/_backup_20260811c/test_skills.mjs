...
const expect = { s01: 0, s03: 1, s06: 3, s09: 5, s14: 6, s32: 10, s39: 24, s38: 72, s40: 72, s47: 72 };
...
for (const [id, exp] of Object.entries(dotChecks)) {
  const s = skills.find((x) => x.id === id);
  t(`${id} dot 持续 ${s.effectRounds[0].durRounds} 回合`, s.effectRounds[0].durRounds === exp);
}

console.log('== Skill 状态机 ==');
const s01 = skills[0]; // cd 0
t('s01 use 不置冷却（cd0）', s01.use() && s01.currentCd === 0 && s01.ready);
const s03 = skills[2]; // cd 1
t('s03 use 置冷却 1 且不可用', s03.use() && s03.currentCd === 1 && !s03.ready);
t('冷却中 use 返回 false', !s03.use());
s03.tick();
t('tick 后冷却归零可用', s03.currentCd === 0 && s03.ready);

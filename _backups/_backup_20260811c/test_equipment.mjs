...
// 验证：生成器（atk/def 属性体系/品质 color/词缀范围/orbSlots/baseScore/score）
...
import {
  ...
  rollDropChoices, redChancePctFor, redPityAfter,
  RED_BASE_PCT, RED_PITY_STEP, RED_FLOOR_100, RED_NO_SET_BONUS,
...
console.log('== C. 评分公式（末光三套） ==');
{
  ...
  const armor = { slot: 'chest', stats: { def: 10, haste: 2 } };
  t('C2 防具 def×10+副×0.5', gearBaseScore(armor) === Math.floor(10 * 10 + 2 * 0.5));
  const acc = { slot: 'ring', stats: { atk: 5, def: 5, crit: 2, haste: 2, versa: 2 } };
  t('C3 首饰 (atk+def)×5+副×1', gearBaseScore(acc) === Math.floor(5 * 5 + 5 * 5 + 2 + 2 + 2));
}
...
console.log('== K. 战斗掉落候选（方案 3：3 件 3 选 1 + 红装保底曲线） ==');
{
  const c0 = rollDropChoices(constRng(0.5), { floor: 1, depth: 1, redChancePct: 0 });
  t('K1 默认 3 件候选', c0.length === 3, `n=${c0.length}`);
  // 红判定：redChancePct=100 → 全终焉(8)；0 → 正常品质曲线（floor=1 不出红）
  const allRed = rollDropChoices(constRng(0.5), { floor: 1, depth: 1, redChancePct: 100 });
  t('K2 redChancePct=100 → 全终焉', allRed.length === 3 && allRed.every((g) => g.rarityIdx === 8));
  const noRed = rollDropChoices(constRng(0.5), { floor: 1, depth: 1, redChancePct: 0 });
  t('K3 redChancePct=0 → 正常品质曲线（无终焉）', noRed.every((g) => g.rarityIdx < 8));
  // 1% 红判定边界：rng×100 < redChancePct
  const redLow = rollDropChoices(() => 0.005, { floor: 1, depth: 1, redChancePct: 1 });
  t('K4 1% 红：低 rng 命中终焉', redLow[0].rarityIdx === 8);
  const redHigh = rollDropChoices(() => 0.5, { floor: 1, depth: 1, redChancePct: 1 });
  t('K5 1% 红：高 rng 不命中（正常品质）', redHigh[0].rarityIdx < 8);
  // minRarity（Boss 史诗保底）
  const bossChoices = rollDropChoices(constRng(0.5), { floor: 1, depth: 1, minRarity: 4, redChancePct: 0 });
  t('K6 minRarity=4 → 全部史诗+', bossChoices.every((g) => g.rarityIdx >= 4));
  // count 可配
  const five = rollDropChoices(constRng(0.5), { floor: 1, depth: 1, count: 5 });
  t('K7 count 可配（5 件）', five.length === 5);
  // 红装保底常量（方案 3 定案值）
  t('K8 保底常量（基础 1% / 步进 0.5 / 100 层后 3% / 未凑齐 +2%）', RED_BASE_PCT === 1 && RED_PITY_STEP === 0.5 && RED_FLOOR_100 === 3 && RED_NO_SET_BONUS === 2);
  // 红装强度公式成长（floorScale 随层数）
  const redLowFloor = rollEquipment(rng, { floor: 1, forceRarity: 8, forceSlot: 'weapon' });
  const redHighFloor = rollEquipment(rng, { floor: 100, forceRarity: 8, forceSlot: 'weapon' });
  t('K9 红装强度随层数公式成长（100 层武器主属性显著更高）', redHighFloor.stats.atk > redLowFloor.stats.atk * 3, `low=${redLowFloor.stats.atk} high=${redHighFloor.stats.atk}`);
}

console.log('== K2. 保底核心纯函数（方案 3 数值心脏） ==');
{
  const noSet = () => ({ redPity: 0, finaleCollection: { weapon: true, head: true, chest: true, legs: true, feet: true, pendant: true, ring: true, trinket: true } }); // 已凑齐一套红
  const noCollection = () => ({ redPity: 0, finaleCollection: {} }); // 未凑齐
  // 基础 1%（100 层前）
  t('K10 1 层 + 已凑齐 → 1%', redChancePctFor(noSet(), 1) === 1);
  t('K11 99 层 + 未凑齐 → 仍 1%（100 层前无加成）', redChancePctFor(noCollection(), 99) === 1);
  // 100 层后：基础 3%；未凑齐 +2% = 5%
  t('K12 100 层 + 已凑齐 → 3%', redChancePctFor(noSet(), 100) === 3);
  t('K13 100 层 + 未凑齐 → 5%（3+2）', redChancePctFor(noCollection(), 100) === 5);
  // pity 叠加（未凑齐 + pity 4.5 → 9.5）
  t('K14 pity 叠加（100 层未凑齐 + pity 4.5 → 9.5）', redChancePctFor({ redPity: 4.5, finaleCollection: {} }, 100) === 9.5);
  // pity 更新：出红（候选含终焉）→ 重置 0；未出红 → +0.5
  const withRed = [{ rarityIdx: 8 }, { rarityIdx: 2 }, { rarityIdx: 5 }];
  const noRed = [{ rarityIdx: 4 }, { rarityIdx: 2 }, { rarityIdx: 5 }];
  t('K15 出红 → pity 重置 0', redPityAfter(withRed, 3) === 0);
  t('K16 未出红 → +0.5（pity 3 → 3.5）', redPityAfter(noRed, 3) === 3.5);
  t('K17 pity 缺省 0 → 0.5', redPityAfter(noRed) === 0.5);
}

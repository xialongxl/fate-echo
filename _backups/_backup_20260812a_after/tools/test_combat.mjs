...
}
{
  ...
  e2.startTurn();
  // 终焉 s51 独占：dot 先回合挂载，下回合领域（分回合流程）
  e2.queueMain('s04');
...
}
{
  ...
  e.queueMain('s04');
  e.confirm();
  t('K10 dot 4 倍（强化期间两回合各：立即伤 + 双结算 2×30）', e.enemies[0].hp === 300 - 2 * (expectDmg(0.5) + 2 * expectDmg(1.6)), `hp=${e.enemies[0].hp}`);
}
{
...
{
  // 敌人终焉主行动后跳过瞬发（终焉消耗全部行动；审查 F 项补测）

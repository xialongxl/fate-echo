...
console.log('== A. 经验曲线 ==');
{
  t('A1 Lv1 经验 = floor(100×1.035^0) = 100', expToLevel(1) === 100, `v=${expToLevel(1)}`);
  t('A2 Lv2 = floor(100×1.035) = 103', expToLevel(2) === Math.floor(100 * 1.035));
...
console.log('== D. 升级结算 ==');
{
  ...
  p.level = 1; p.exp = 0;
  r = grantExp(p, 100 + 103 + 10); // 连升 2 级余 10
  t('D2 213 经验 → 连升 2 级（Lv3 余 10）', p.level === 3 && p.exp === 10 && r.levelsGained === 2);
  p.level = 1; p.exp = 0;
  r = grantExp(p, 50);

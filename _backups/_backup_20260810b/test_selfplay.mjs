...
check('奖励三选一结算（战利品）', panelText().includes('战利品'), panelText().slice(0, 60));
check('掉落闭环（自动穿/入包/无掉落均可）', panelText().includes('掉落') ? (fe().game.player.inventory.length > 0 || Object.values(fe().game.player.equipment).filter(Boolean).length > 0) : true);
console.log(`  结算: ${panelText().replace(/\s+/g, ' ').slice(0, 90)}`);
...
const optBtn = findBtn(el('ow-panel'), '打开') || findBtn(el('ow-panel'), '祈祷') || findBtn(el('ow-panel'), '小心') || findBtn(el('ow-panel'), '休息') || findBtn(el('ow-panel'), '无视') || findBtn(el('ow-panel'), '强行');
if (optBtn) {
  fireClick(optBtn);
...
fireClick(findBtn(el('ow-panel'), '抽取一次'));
...
const slotRowBtn = (slotName, btnText) => {
  const lists = (el('ow-panel').children || []).filter((n) => n.className === 'bag-list');

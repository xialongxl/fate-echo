...
// Fate_echo 适配：floor = (areaIndex+1) + (depth-1)×10（塔层缩放映射到楼层）；
//   属性体系 atk/int(防御力)/haste/crit/versa（int 接入战斗减伤公式，与末光一致）
// 装备对象结构（末光 gear 完整字段）：
//   { id, name, slot, rarityIdx, stats{atk,int,haste,crit,versa}, enhanceLv,
...
// 主属性：武器 atk=bg×3.0 ｜ 防具 int=bg×2.0 ｜ 首饰 atk=int=bg×0.4
// 词缀：haste/crit/versa 按部位范围随机（保留小数）；单件上限保护
// orbSlots：终焉3 / 传说+2 / 精良+1 / 其他0；终焉自动 locked
export function rollEquipment(rng = Math.random, { floor = 1, depth = 1, forceRarity = null, forceSlot = null } = {}) {
  const rIdx = forceRarity !== null ? forceRarity : rollRarity(rng, floor);
...
// ---- 评分（末光三套公式照抄） ----
// 武器 atk×10 + 副×0.5 ｜ 防具 int×10 + 副×0.5 ｜ 首饰 (atk+int)×5 + 副×1
export function gearBaseScore(gear) {
  ...
  if (isAccessory(gear.slot)) {
    return Math.floor((s.atk || 0) * 5 + (s.int || 0) * 5 + (s.haste || 0) * 1 + (s.crit || 0) * 1 + (s.versa || 0) * 1);
  }
  return Math.floor((s.int || 0) * 10 + (s.haste || 0) * 0.5 + (s.crit || 0) * 0.5 + (s.versa || 0) * 0.5);
}
...
export function enhanceGear(player, id, rng = Math.random) {
  ...
  if (success) {
    gear.enhanceLv++;
    // 主属性：×(1.05 + 等级×0.03) + 固定加值（武器 atk+5 / 防具 int+5 / 首饰 atk+2 且 int+2）
...
// 末光 recalcStats 对应：装备 stats 按槽位汇总（属性体系 atk/int/haste/crit/versa）
export function totalEquipmentStats(equipment = {}) {
  const sum = { atk: 0, int: 0, crit: 0, haste: 0, versa: 0 };
  for (const slot of SLOTS) {

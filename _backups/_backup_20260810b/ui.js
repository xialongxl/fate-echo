...
export class BattleUI {
    ...
    for (let i = 0; i < e.enemies.length; i++) {
      ...
      card.innerHTML = `
        ...
        <div class="unit-stats">
          <span>攻 <b>${esc(u.atk)}</b></span>
          <span>智 <b>${esc(u.int)}</b></span>
        </div>
  ...
  _enemyChips(u) {
    ...
    for (const b of u.buffs) chips.push(`<span class="chip" title="增益">${esc(b.stat)} +${b.val} ${fmtTurns(b.turns)}</span>`);
  ...
  _renderPlayer() {
    ...
    panel.innerHTML = `
      ...
      <div class="unit-stats">
        <span>攻 <b>${esc(p.atk)}</b></span>
        <span>暴击 <b>${(p.totalCritChance() * 100).toFixed(0)}%</b></span>
        <span>急速 <b>${esc(p.statBonus('haste'))}</b></span>
        <span>共鸣 <b>${esc(p.statBonus('versa'))}</b></span>
      </div>
  ...
  _playerChips(p) {
    ...
    for (const b of p.buffs) chips.push(`<span class="chip" title="增益">${esc(b.stat)} +${b.val} ${fmtTurns(b.turns)}</span>`);
  ...
  _renderSkills() {
    ...
    const preview = (skill) => {
      ...
      if (eff && eff.type === 'heal') return `治疗≈${Math.round(Math.max(p.atk, p.int) * (eff.val || 0))}`;
...
export class OverworldUI {
  constructor(els, handlers) {
    this.els = els;           // {owPlayer, owTower, owNodes, owPanel, toast?}
    this.handlers = handlers; // {onNode(i), onOption(id), onBuy(id), onSell(id), onGacha(), onNewGame(), onContinue(), onLeave()}
    this._toastTimer = null;
  }
  ...
  renderShop(game, stock, priceFn = priceOf) {
  ...
  _renderGearDetail(gear, isEquipped) {
    ...
    statsEl.innerHTML = `
      <div class="stats-box">
        ...
        ${s.int !== undefined ? `<span><span class="label">防御力(智)</span><br><b>${fmtNum(s.int)}</b></span>` : ''}
        ${s.crit ? `<span><span class="label">暴击率</span><br><b>${fmtNum(s.crit)}%</b></span>` : ''}
        ${s.haste ? `<span><span class="label">冷却缩减</span><br><b>${fmtNum(s.haste)}%</b></span>` : ''}
        ${s.versa ? `<span><span class="label">共鸣</span><br><b>${fmtNum(s.versa)}%</b></span>` : ''}
        ${orbLines}
    ...
    if (gear.rarityIdx === 8) {
      mkBtn('✨ 终焉精炼', 'btn btn-ghost', () => { this.closeBackpack(); this.handlers.onRefine(gear.id); });
    }
  }
  ...
  _renderEnhanceDetail(gear) {
    ...
    const attrs = [
      ...
      s.int !== undefined ? `防御力(智) ${fmtNum(s.int)}` : '',
      s.crit ? `暴击 ${fmtNum(s.crit)}%` : '',
      s.haste ? `急速 ${fmtNum(s.haste)}%` : '',
      s.versa ? `共鸣 ${fmtNum(s.versa)}%` : '',
    ].filter(Boolean).join(' ｜ ');
    detail.innerHTML = `
  ...
  _renderRefineDetail(gear) {
    ...
    for (const aff of AFFIXES_REF) {
      ...
      html += `<div style="display:flex;align-items:center;gap:10px;margin-bottom:8px;">
        <span style="flex:1">${AFF_NAME_REF[aff] || aff}：<b>${fmtNum(val)}</b> / ${cap}
          <span class="node-sub">（精炼 ${lv}/${REFINE_MAX_REF}，进度 ${pct}%）</span></span>
        <button class="btn btn-ghost ${canRefine ? '' : 'enhance-disabled'}" data-aff="${aff}">精炼</button>
      </div>`;
    }
  ...
  renderGlossary(game, stats = null) {
    this.renderPanel('📖 术语表 · 帮助', (panel) => {
      ...
      const terms = [
        ...
        ['装备属性', '攻击 atk / 防御(智) int（参与减伤）/ 暴击 / 急速 / 共鸣'],
        ['装备评分', '武器 atk×10、防具 int×10、首饰 (atk+int)×5，副属性加成；score = baseScore + 宝珠分'],
  ...
  // 战斗胜利奖励结算（杀戮尖塔式：掉落展示 + 奖励三选一）
  renderBattleResult(game, lines, choices) {
    this.renderPanel('🎁 战利品', (panel) => {
      for (const l of lines) {
        const d = document.createElement('div');
        d.className = 'panel-text';
        d.textContent = l;
      ...
      if (choices && choices.length) {
        ...
        choices.forEach((c, i) => {
          ...
          b.className = 'opt-btn';
          if (c.kind === 'gear') {
            b.innerHTML = `⚔ 装备<br><span class="node-sub">${esc(c.gear.name)}（${rarityName(c.gear.rarityIdx)}）评分 ${c.gear.score}</span>`;
          } else if (c.kind === 'gold') {
            b.innerHTML = `💰 金币<br><span class="node-sub">+${c.amount} 金</span>`;
          } else {
            b.innerHTML = `🔥 篝火<br><span class="node-sub">+${c.amount} 生命</span>`;
          }
  ...
  _renderPlayer(game) {
    const p = game.player;
    const stats = playerStatsAtRef(p.level);
    ...
    this.els.owPlayer.innerHTML = `
      ...
      <div class="ow-row"><span>生命</span><b>${p.hp}/${stats.hp + eqStats(eq).hp}</b></div>
      <div class="ow-row"><span>法力</span><b>${p.mp}/${stats.mp}</b></div>
      <div class="ow-row"><span>攻击</span><b>${stats.atk + eqStats(eq).atk}</b></div>
      <div class="ow-row"><span class="gold-line">金币</span><b class="gold-line">${p.gold}</b></div>
      <div class="eq-slot">${SLOTS_REF.map((s) => `${SLOT_NAMES_REF[s]}：${eq[s] ? esc(eq[s].name) : '—'}`).join(' ｜ ')}</div>`;
  }
...
import { playerStatsAt as playerStatsAtRef, expToLevel as expToLevelRef } from './progression.js';
...
const statsText = (gear) => {
  const parts = Object.entries(gear.stats || {}).map(([k, v]) => `${k === 'crit' ? '暴击' : k === 'haste' ? '急速' : k === 'versa' ? '共鸣' : k === 'atk' ? '攻击' : k === 'int' ? '防御' : '生命'}+${fmtNum(v)}`);

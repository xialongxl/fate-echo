...
console.log('== A. 启动与区域地图 ==');
{
  ...
  t('A4b 局外属性面板 = 末光 stats-grid UI 形式（stat-item 卡片六项 + GCD）', (() => {
    ...
    return h.includes('stat-item') && h.includes('攻击力') && h.includes('防御力') && h.includes('急速') && h.includes('暴击率') && h.includes('共鸣') && !h.includes('GCD') && !h.includes('eq-slot');
...
console.log('== J. 失败与读档 ==');
{
  ...
  fe().newGame();
  // 构造必败：玩家 1 血进 Boss 战（s45 命运轮转带 8 回合冷却，仅一次回满 → Lv1 输出打不动 Boss → 必死）
  fe().game.player.hp = 1;
  fe().game.player.mp = 0;
...
console.log('== R. 背包详情 + 属性显示（对齐末光） ==');
{
  ...
  t('R8 属性行显示急速/共鸣数值（属性全称"攻击/防御"；GCD 已删）', playerHtml.includes('急速') && playerHtml.includes('共鸣') && playerHtml.includes('攻击') && playerHtml.includes('防御') && !playerHtml.includes('GCD'), playerHtml.slice(0, 200));
...
console.log('== T. 局外技能配置（即时保存 + 战斗生效 + 主技能守卫） ==');
{
  ...
  t('T13 摘要行保留（倍率 ×；MP/CD 已删）', descText(el('panel-body')).includes('倍率 ×1') && !descText(el('panel-body')).includes('CD'), descText(el('panel-body')).slice(0, 200));
  fireClick(el('panel-close'));
}

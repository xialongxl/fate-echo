...
console.log('== C. 施放与消耗 ==');
{
  ...
  t('C3 s03 去 CD 后无冷却（currentCd 恒 0）', e.turn === 2 && e.playerSkills.find((s) => s.id === 's03').currentCd === 0);
...
{
  // 彻底去 CD（2026-08-11 用户定案）：s07 无冷却，跨回合可重复入队施放
  const e = eng();
  e.startTurn();
  e.queueInstant('s07');
  e.confirm();
  e.startTurn();
  t('C6 无冷却：s07 下回合可再次入队', e.queueInstant('s07').ok, `reason=${e.queueInstant('s07').reason}`);
  e.queueMain('s01');
  e.confirm();
  t('C7 currentCd 恒 0（无冷却）', e.playerSkills.find((s) => s.id === 's07').currentCd === 0, `cd=${e.playerSkills.find((s) => s.id === 's07').currentCd}`);
}
{
...
{
  // 2026-08-11 去 CD + 终焉独占：s41 虚空化身（终焉）独占回合——s04 无法同回合入队
...
}
{
  ...
  e.queueMain('s04');
  e.confirm();
  e.queueInstant('s38'); // 终焉 ogcd 入队（瞬发槽被终焉独占）
...
{
  // 持续效果刷新（去 CD 配套：同源刷新不叠加）；haste 15 → AP 2；敌人 atk 0（不干扰盾量）

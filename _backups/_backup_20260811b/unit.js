...
// 属性：hp/mp/atk/def/int（int 参与减伤公式，NEW_GAME.md §2.3；
//       def 暂未接入任何伤害公式，保留为属性占位，Phase 2 数值平衡时定用法）
// 状态：buffs / dots / hots / shield / immune / vulnMult+vulnTurns / dotEnhanced
// ============================================================

export class CombatUnit {
  constructor({ name = '单位', hp = 100, maxHp = 100, mp = 50, maxMp = 50, atk = 10, def = 0, int = 10, level = 1, critChance = 0.05, dotImmune = false, enraged = false } = {}) {
    this.name = name;
    this.hp = hp;
    this.maxHp = maxHp;
    this.mp = mp;
    ...
    // 持续状态（回合制）
    this.buffs = [];        // { key, stat, val, turns }（stat: versa/haste/crit/dmg_up_pct）

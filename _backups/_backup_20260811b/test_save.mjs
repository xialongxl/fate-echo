...
console.log('== B. 版本迁移 ==');
{
  t('B1 v3→v4→v5→v6 迁移链存在', MIGRATIONS[2] && MIGRATIONS[3] && MIGRATIONS[4] && MIGRATIONS[5] && typeof MIGRATIONS[2] === 'function' && typeof MIGRATIONS[3] === 'function' && typeof MIGRATIONS[4] === 'function' && typeof MIGRATIONS[5] === 'function');
...
console.log('== F. 存储隔离 ==');
{
  ...
  t('F2 存档版本号 v6（含 area/finaleEssence/finaleCollection/redPity/battleSeed/collections）', SAVE_VERSION === 6 && defaultGameState().area !== undefined && defaultGameState().player.finaleEssence === 0 && defaultGameState().player.redPity === 0 && defaultGameState().player.battleSeed === null && Array.isArray(defaultGameState().player.collections));
}

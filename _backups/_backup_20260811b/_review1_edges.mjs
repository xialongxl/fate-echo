...
//       calcDamage 边界（0 倍率、int=0 除零、crit>1）/
...
t('s03 冷却 1 回合', s03.cdRounds === 1);
t('s03 首次 use → true', s03.use() === true);
t('s03 冷却中连续 use → false', s03.use() === false && s03.currentCd === 1);
t('s03 冷却中 use 不改变 currentCd', s03.currentCd === 1);
s03.tick();
t('s03 tick 一次归零可用', s03.ready && s03.currentCd === 0);
const s09 = new Skill(SKILLS_DB.find((s) => s.id === 's09'));
t('s09 冷却 5 回合（12000ms）', s09.cdRounds === 5);
t('s09 use → currentCd 5', s09.use() && s09.currentCd === 5);
for (let i = 0; i < 4; i++) s09.tick();
t('s09 tick×4 → 剩 1', s09.currentCd === 1 && !s09.ready);
s09.tick();
t('s09 tick×5 → 0 可用', s09.currentCd === 0 && s09.ready);
s09.tick();
t('s09 冷却已 0 时 tick 不越界', s09.currentCd === 0);
...
console.log('== D. calcDamage 边界 ==');
{
  ...
  const zi = calcDamage(src, mkTarget({ int: 0 }), 1.0, { noCrit: true });
  t('target.int=0（lv10）→ 无减伤 100', zi === 100, `got ${zi}`);
  const zz = calcDamage(src, mkTarget({ int: 0, level: 0 }), 1.0, { noCrit: true });
  t('target.int=0 且 level=0 → 100（防 0/0 NaN）', zz === 100 && !Number.isNaN(zz), `got ${zz}`);
  const lv0 = calcDamage(src, mkTarget({ level: 0 }), 1.0, { noCrit: true });
  t('target.level=0（int10）→ 减伤封顶 85% → 15', lv0 === 15, `got ${lv0}`);
  const neg = calcDamage(src, mkTarget({ int: -5 }), 1.0, { noCrit: true });
  t('target.int=-5 → 负减伤夹取 0 → 100', neg === 100, `got ${neg}`);
  ...
  let all150 = true;
  for (let i = 0; i < 50; i++) {
    if (calcDamage(crit2, mkTarget({ int: 0 }), 1.0, { noMitigation: true }) !== 150) all150 = false;
  }
...
console.log('== E. executeSkillEffects 单次结算 / 空效果 / 容错 ==');
{
  ...
  // s30：dmgMult 2.0 + 回蓝效果并存（回蓝自增益 → 施放者）
  const u3 = mkUnit({ mp: 150 }); const e3 = mkTarget({ hp: 300, mp: 150 });
  logs.length = 0;
  executeSkillEffects(ctx, new Skill(SKILLS_DB.find((s) => s.id === 's30')), u3, e3);
  t('s30 伤害 195 + 回蓝 10 在施放者（target 不受回蓝）', e3.hp === 105 && e3.mp === 150 && u3.mp === 160, `e3=${e3.hp}/${e3.mp}, u3=${u3.mp}`);
...
const FIELDS = ['id', 'name', 'reqLv', 'type', 'cd', 'cost', 'dmgMult', 'priority', 'desc', 'conditionMaxHPPct'];
...
for (let i = 0; i < Math.max(orig.length, SKILLS_DB.length); i++) {
  ...
  if (norm(o.effects) !== norm(n.effects)) {
    effMismatch++;
    mismatches.push(`${n.id}.effects: 源=${JSON.stringify(o.effects)} 现=${JSON.stringify(n.effects)}`);
  }
}
t('逐字段对照无差异（53×11 字段 + effects）', fieldMismatch === 0 && effMismatch === 0, mismatches.slice(0, 5).join(' | '));

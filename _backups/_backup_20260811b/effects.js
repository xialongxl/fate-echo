...
export function calcDamage(source, target, dmgMult, opts = {}) {
  ...
  // 敌人减伤：int / (int + 等级×40)，上限 85%
  if (!noMitigation) {
    const denom = target.int + target.level * 40;
    const m = Number.isFinite(target.int) && denom > 0 ? target.int / denom : 0; // 防 0/0 与负分母
    const mitigation = Math.min(0.85, Math.max(0, m)); // 负 int 不产生负减伤
    dmg *= 1 - mitigation;
  }
...
export const EFFECT_HANDLERS = {
  ...
  // 立即治疗（末光照抄：基数 = max(atk, int)）
  heal(ctx, target, source, e) {
    const before = target.hp;
    target.heal(Math.max(source.atk, source.int) * (e.val || 0));

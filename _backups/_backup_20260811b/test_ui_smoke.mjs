...
console.log('== R. 背包详情 + 属性显示（对齐末光） ==');
{
  ...
  const gear = { id: 'g_det', name: '测试 法袍', slot: 'chest', rarityIdx: 2, stats: { int: 20, haste: 9.940000000000001, crit: 7.5 }, enhanceLv: 0, locked: false, pinned: false, score: 88, baseScore: 88, orbs: [], refineLevels: {}, refineInitialValues: {} };
  ...
  t('R7 永久装备/被动 buff 不进 chips（haste/versa/dmg_up_pct 无 ∞ 行）', !playerHtml.includes('haste +') && !playerHtml.includes('versa +') && !playerHtml.includes('dmg_up_pct'), playerHtml.slice(0, 200));
  t('R8 属性行显示急速/共鸣数值', playerHtml.includes('急速') && playerHtml.includes('共鸣'));
  fe().newGame(); // 收尾
}

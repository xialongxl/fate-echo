...
console.log('== B. 版本迁移 ==');
{
  t('B1 v3→v4→v5→v6→v7→v8 迁移链存在', MIGRATIONS[2] && MIGRATIONS[3] && MIGRATIONS[4] && MIGRATIONS[5] && MIGRATIONS[6] && MIGRATIONS[7] && typeof MIGRATIONS[2] === 'function' && typeof MIGRATIONS[3] === 'function' && typeof MIGRATIONS[4] === 'function' && typeof MIGRATIONS[5] === 'function' && typeof MIGRATIONS[6] === 'function' && typeof MIGRATIONS[7] === 'function');
  ...
  // skillLoadout 白名单：合法保留 / 去重 / 畸形回退 null / 空 main 回退 null
  const loOk = sanitizeSave({ version: 8, player: { level: 3, skillLoadout: { main: ['s01', 's03', 's01', 42], instant: ['s07'] } } });
  t('B1k skillLoadout 白名单：字符串过滤 + 去重保留', loOk.player.skillLoadout.main.join() === 's01,s03' && loOk.player.skillLoadout.instant.join() === 's07', JSON.stringify(loOk.player.skillLoadout));
  ...
  t('B1m skillLoadout 空 main → null（至少 1 个主技能）', loEmpty.player.skillLoadout === null);

...
export function defaultGameState() {
  return {
    ...
    player: {
      ...
      skillLoadout: null, // 技能配置 { main: [id], instant: [id] }；null = 全部解锁（局外技能配置面板）
    },
...
// 技能配置白名单（2026-08-11 局外技能配置）：
//   合法 = { main: string[], instant: string[] }（id 字符串过滤 + 去重 + 上限 64 防膨胀 +
//   SKILLS_DB 存在性校验——防手工伪造存档软锁战斗）
//   畸形/缺结构 → null（全部解锁，现状兼容）；空 main → null（至少保留 1 个主技能）
//   注：9/4 上阵上限由配置面板 UI 守卫（添加时拒绝），sanitize 不截断——
...
function sanitizeSkillLoadout(v) {
  ...
  const clean = (arr) => Array.isArray(arr)
    ...
    : [];
  const main = clean(v.main);
  const instant = clean(v.instant);

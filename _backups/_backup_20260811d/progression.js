...
//   loadout = { main: [skillId], instant: [skillId] } ｜ null = 全部解锁（默认）
//   过滤：主技能 = 非瞬发非被动（gcd/buff/dot/debuff/domain，与 ui.js/autoloop 的
//   mains 定义一致）；瞬发 = ogcd；被动始终保留（不参与配置）。
//   id 不存在自动忽略，乱序无碍。用 type 字段判定（兼容 Skill 实例与原始数据对象）
export function applySkillLoadout(skills, loadout) {
  if (!loadout || !Array.isArray(loadout.main) && !Array.isArray(loadout.instant)) return skills;
  const selectedMain = new Set(Array.isArray(loadout.main) ? loadout.main : []);
  const selectedInstant = new Set(Array.isArray(loadout.instant) ? loadout.instant : []);
  return skills.filter((s) => {
    if (s && s.type === 'passive') return true;                     // 被动常驻（终焉之力）
    if (s && s.type === 'ogcd') return selectedInstant.has(s.id);   // 瞬发（每回合 1 槽）
    return selectedMain.has(s.id);                                  // 其余全部主技能
  });
}

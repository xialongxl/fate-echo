...
console.log('== F. 技能配置（applySkillLoadout，2026-08-11） ==');
{
  const all = unlockedSkillData(100); // 全解锁（53；原始数据对象，用 type 判断）
  const isMain = (s) => s.type !== 'ogcd' && s.type !== 'passive';
  const isInstant = (s) => s.type === 'ogcd';
  const isPassive = (s) => s.type === 'passive';
  t('F1 null → 全部保留（现状兼容）', applySkillLoadout(all, null).length === all.length);
  const anyMain = all.find(isMain);
  const anyInstant = all.find(isInstant);
  const out = applySkillLoadout(all, { main: [anyMain.id], instant: [anyInstant.id] });
  t('F2 只保留选中主/瞬发 + 被动常驻', out.length === 3
    && out.some((s) => s.id === anyMain.id) && out.some((s) => s.id === anyInstant.id) && out.some(isPassive),
    out.map((s) => s.id).join(','));
  t('F3 不存在 id 自动忽略（只剩被动）', applySkillLoadout(all, { main: ['nope_skill'], instant: [] }).every(isPassive));
  t('F4 空 main → 只剩被动（战斗端无主技能）', applySkillLoadout(all, { main: [], instant: [] }).every(isPassive));
  // 战斗集成：makeBattlePlayer 应用配置（buff/dot 等非 gcd 主技能同样被过滤）
  ...
  const onlyHeal = makeBattlePlayer(mkState({ main: ['s05'], instant: [] })).skills;
  t('F5 配置后战斗技能组 = 主 1 + 被动（s05 治疗 + 终焉之力）', onlyHeal.length === 2 && onlyHeal.some((s) => s.id === 's05') && onlyHeal.some(isPassive), onlyHeal.map((s) => s.id).join(','));

...
function onBattleEnd({ result }) {
  ...
  if (result === 'victory') {
    ...
    // 金酒之杯（250 累计）：战斗金币 +10%
    gold = Math.round(gold * goldBonusRate(atmTotalCache));
    const up = grantExp(game.player, exp);
    game.player.gold += gold;
    // 篝火全选：自动恢复 30% 最大生命（原三选一"篝火"并入全选）
    ...
    const choices = rollDropChoices(battleRng, { floor: floorNum(), depth: area.depth, minRarity: bossTier ? 4 : null, redChancePct: redChance });
...
function openExit() {
  ...
  // 金酒之杯（250 累计）：出口金币 +10%
  game.player.gold += Math.round(goldReward * goldBonusRate(atmTotalCache));
  // 篝火全选：自动恢复 30% 最大生命
  const maxHp = makeBattlePlayer(game).unit.maxHp;
  const healed = Math.round(maxHp * 0.3);
  game.player.hp = Math.min(maxHp, game.player.hp + healed);
  ...
  const lines = [
    `🚪 抵达${isRare ? '险路小径' : '险路尽头'}：金币 +${Math.round(goldReward * goldBonusRate(atmTotalCache))}，篝火恢复 ${healed} 生命（自动入账）`,
...
async function openShop() {
  ...
  const priceFn = (gear) => shopPriceFor(gear, discountRate(atm.total));
  owUI.renderShop(game, stock, priceFn, atm);

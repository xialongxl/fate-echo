...
export class AreaMapView {
   ...
   * @param {object} opts
   * @param {(nodeId: string) => void} opts.onMove  点击可达节点回调（移动）
   * @param {() => void} opts.onEnter  点击当前节点回调（进入确认层，DESIGN_NOTES 第 1 条）
   * @param {number} opts.cellSize  节点间距像素（默认 64）
   */
  constructor(viewport, { onMove = () => {}, onEnter = () => {}, cellSize = 64 } = {}) {
  ...
  _bindViewport() {
    ...
    this.viewport.addEventListener('pointerdown', (e) => {
      ...
      // 点击判定：可达邻居（walkable）→ 移动；当前节点（current）→ 进入确认层
      const isCurrent = !!(nodeEl && nodeEl.classList && nodeEl.classList.contains('current'));
      const isWalkable = !!(nodeEl && nodeEl.classList && nodeEl.classList.contains('walkable'));
      this._press = {
        node: isCurrent || isWalkable ? nodeEl : null,
        kind: isCurrent ? 'enter' : 'move',
    ...
    const endDrag = (e) => {
      ...
      this._dragging = null;
      this._press = null;
      // 位移小（未拖拽）且按在可交互节点上 → 点击分发（当前节点 → 确认层；可达节点 → 移动）
      if (press && press.node && e && Math.abs(e.clientX - press.x) < 6 && Math.abs(e.clientY - press.y) < 6) {
        if (press.kind === 'enter') this.onEnter();
        else this.onMove(press.node.dataset.id);
      }
    };

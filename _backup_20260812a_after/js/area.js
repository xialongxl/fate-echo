...
import { createEnemy, createRandomEnemy, ENEMIES_DB } from './enemies.js';
...
export class Area {
  ...
  buildEncounters(node = this.current()) {
    ...
    if (node.type === 'exit_boss') {
      // 险路恶敌：随机 Boss（12 模板技能/机制 + 守卫者头衔 + 数值浮动，F3）
      return [createRandomEnemy({ rng: this.rng, level, depth: this.depth, tier: 'boss' })];
    }
    ...
    if (node.type === 'elite') {
      // 紧急作战：精英怪（后期 30% 双敌）；20% 概率随机精英（名字/数值浮动，机制随模板）
      ...
      const spawn = () => (this.rng() < 0.2
        ? createRandomEnemy({ rng: this.rng, level, depth: this.depth, tier: 'elite' })
        : createEnemy(pick(), level));
      return Array.from({ length: count }, spawn);
    }
    // 普通区：前 2 区域单敌，第 3 区域起 20% 双敌；前 2 区域无精英；
    // 30% 概率随机怪（黑流树海"异兽出没"多样性，F3）
    ...
    const pick = () => (elites.length && this.rng() < eliteWeight)
      ? elites[Math.floor(this.rng() * elites.length)].id
      : normals[Math.floor(this.rng() * normals.length)].id;
    const spawn = () => (this.rng() < 0.3
      ? createRandomEnemy({ rng: this.rng, level, depth: this.depth, tier: 'normal' })
      : createEnemy(pick(), level));
    return Array.from({ length: count }, spawn);
  }

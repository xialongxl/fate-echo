...
function onBattleEnd({ result }) {
  ...
  if (ai && settings.aiLearning) {
    ai.recordBattle(result === 'victory'); // 战斗记录（永恒回想录 onBattleEnd 照抄：胜/负计数）
    ai.save().catch((err) => console.warn('SmartAI 保存失败:', err));
  }
...
function doRest() {
  ...
  area.clearCurrent();
  persist();
  owUI.renderResult(game, [`篝火温暖了身体：恢复 ${healed} 生命。`], '返回地图', () => { owMode = 'select'; renderArea(); });
}
...
// 统一 AI 学习开关（面板 + 战斗内 chk-ai 共用；审查 L5 提取防发散）
//   关闭时清 _lasts（防跨战斗样本污染：上场的 (state,action) 残留会污染下场 reward——审查 A #2）
function setAILearning(enabled, ui = null) {
  settings.aiLearning = enabled;
  settings.save();
  if (engine) engine.ai = enabled ? ai : null;
  if (!enabled && ai && ai._lasts) ai._lasts.clear();
  if (enabled && !ai) (ui || battleUI).toast('SmartAI 不可用（TF.js 加载失败），保持启发式 AI');
  else (ui || battleUI).toast(enabled ? '敌人 AI 学习已开启' : '敌人 AI 学习已关闭（退回启发式）');
}
document.getElementById('chk-ai').addEventListener('change', (e) => {
  setAILearning(e.target.checked);
});
...
// 局外技能配置（勾选切换即时保存；常规 ≥1 守卫；上限 常规 5/终焉 3）
function openSkillConfig() {
  ...
  owUI.handlers.onToggleSkill = (skillId, group) => {
    const p = game.player;
    // null（全部解锁）→ 转为显式全选（常规/终焉按 desc [终焉] 分流；可超 5/3 上限，
    //   与"全部解锁"现状行为一致）
    let lo = p.skillLoadout;
    if (!lo) {
      const unlocked = unlockedSkillData(p.level);
      lo = {
        regular: unlocked.filter((s) => !/\[终焉\]/.test(s.desc || '') && s.type !== 'passive').map((s) => s.id),
        finale: unlocked.filter((s) => /\[终焉\]/.test(s.desc || '')).map((s) => s.id),
      };
    }
    const list = group === 'finale' ? lo.finale : lo.regular;
    const max = group === 'finale' ? 3 : 5;
    const idx = list.indexOf(skillId);
    if (idx >= 0) {
      // 至少保留 1 个主技能（非瞬发非被动——常规组含瞬发，全取消会让战斗无主行动卡死——R1-A）
      if (group === 'regular' && list.length <= 1) {
        owUI.toast('至少保留 1 个常规技能');
        return;
      }
      const rest = list.filter((x) => x !== skillId);
      const hasMain = rest.some((id) => {
        const d = SKILLS_DB.find((x) => x.id === id);
        return d && d.type !== 'instant' && d.type !== 'passive';
      });
      if (!hasMain) {
        owUI.toast('至少保留 1 个主技能（瞬发不能作为主行动）');
        return;
      }
      list.splice(idx, 1);
    } else {
      if (list.length >= max) { owUI.toast(`${group === 'finale' ? '终焉' : '常规'}技能上阵上限 ${max} 个`); return; }
      list.push(skillId);
    }
    p.skillLoadout = { regular: lo.regular, finale: lo.finale };
    persist();
...
// AI 面板（永恒回想录 SmartAI 面板照抄：状态卡 + 自由开关 + 调试工具）
function openAIPanel() {
  owMode = 'panel';
  owUI.renderAIPanel(ai, settings, {
    onToggleAI: (enabled) => {
      setAILearning(enabled, owUI);
      const chk = document.getElementById('chk-ai');
      if (chk) chk.checked = enabled;
    },
    onClose: () => { owMode = 'select'; renderArea(); }, // 降级分支关闭按钮复位（审查 H1）
    onForceTrain: async () => {
      if (!ai) { owUI.toast('SmartAI 不可用'); return; }
      owUI.toast('强制训练已触发，进度见控制台（F12）');
      try { await ai.forceTraining(); } catch (err) { console.warn('强制训练失败:', err); }
    },
    onClearAI: async () => {
      if (!ai) { owUI.toast('SmartAI 不可用'); return; }
      if (!window.confirm('确定要清除所有 AI 学习数据吗？这将删除所有战斗记录和训练好的模型。')) return;
      await ai.clearAllData();
      owUI.toast('AI 学习数据已清除');
      openAIPanel(); // 重新渲染（全部 handler 重新绑定）
    },
    onExportAI: () => {
      if (!ai) { owUI.toast('SmartAI 不可用'); return; }
      ai.exportData(); // 输出到控制台（F12）
      owUI.toast('训练数据已导出到控制台（F12）');
    },
  });
  document.getElementById('phase-el').textContent = '🧠 SmartAI';
  owUI.handlers.onLeave = () => { owMode = 'select'; renderArea(); };
}
...
window.fateEcho = {
  ...
  renderAIPanel: (aiArg, settingsArg, handlersArg) => owUI.renderAIPanel(aiArg, settingsArg, handlersArg), // 调试/测试：AI 面板（ai=null 降级分支验证）
  setRng(fn) { rng = fn; },

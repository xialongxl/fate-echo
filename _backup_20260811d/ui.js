...
export class OverworldUI {
  ...
  // 局外技能配置（2026-08-11）：从解锁技能中勾选上阵（主技能 ≤9 / 瞬发 ≤4），
  ...
  renderSkillConfig(game) {
    ...
    const selectedMain = new Set(loadout && loadout.main ? loadout.main : []);
    const selectedInstant = new Set(loadout && loadout.instant ? loadout.instant : []);
    const onCount = (list, sel, type) => (loadout
      ? list.filter((s) => sel.has(s.id)).length
      : list.filter((s) => s.reqLv <= lv).length);
    const mains = SKILLS_DB_REF.filter((s) => s.type !== 'ogcd' && s.type !== 'passive').sort((a, b) => a.reqLv - b.reqLv);
    const instants = SKILLS_DB_REF.filter((s) => s.type === 'ogcd').sort((a, b) => a.reqLv - b.reqLv);
    const mainOn = onCount(mains, selectedMain);
    const instantOn = onCount(instants, selectedInstant);
    // 技能描述模板填充（data.js desc 为末光毫秒制模板）：用玩家当前完整属性
    //   （makeBattlePlayer 同源）计算实际数值——{dmg} 约伤害（无减伤近似）、
    //   {heal} 治疗、{shieldVal} 护盾、{mpRecover} 回蓝、{cd}/{dur} 秒
    ...
    const fillDesc = (s) => {
      ...
      const vars = {
        ...
        cost: s.cost || 0,
        cd: s.cd ? s.cd / 1000 : 0,
        dur: e0.dur ? e0.dur / 1000 : 0,
      };
      return (s.desc || '').replace(/\{(\w+)\}/g, (m, k) => (k in vars ? vars[k] : m));
    };
    this.renderPanel(`⚙ 技能配置（主技能 ${mainOn}/${mains.length} · 瞬发 ${instantOn}/${instants.length}）`, (panel) => {
      const hint = document.createElement('div');
      hint.className = 'panel-text';
      hint.textContent = '点击切换上阵（即时保存）；至少保留 1 个主技能；未解锁灰色；被动自动生效；新游戏重置为全部解锁。';
      panel.appendChild(hint);
      ...
      mkGroup('── 主技能（消耗行动点）──', mains, selectedMain, 'main');
      mkGroup('── 瞬发（ogcd，每回合 1 槽，不占行动点）──', instants, selectedInstant, 'instant');
      const note = document.createElement('div');

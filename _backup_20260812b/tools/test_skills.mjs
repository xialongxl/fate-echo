...
const bad1 = validateSkillsDB([{ id: 'x1', reqLv: 1, dmgMult: 1, type: 'main', effects: {} }]);
t('effects 非数组 → 报告错误而非抛异常', bad1.errors.some((m) => m.includes('effects 非法')));
const bad2 = validateSkillsDB([
  { id: 'x2', reqLv: 1, dmgMult: 1, type: 'buff', effects: [{ type: 'buff', stat: 'versa', val: 'abc', dur: 15000 }] },
  { id: 'x3', reqLv: 1, dmgMult: 1, type: 'dot', effects: [{ type: 'dot', dps: NaN, dur: -5000 }] },
  { id: 'x9', reqLv: 1, dmgMult: 1, type: 'buff', effects: [{ type: 'hot', pct: 'bad', dur: 10000 }] },
  { id: 'x10', reqLv: 1, dmgMult: 1, type: 'instant', effects: [{ type: 'shield', hpPct: NaN, dur: 10000 }] },
  { id: 'x11', reqLv: 1, dmgMult: 1, type: 'main', effects: [{ type: 'hp_sacrifice', costPct: -0.5, dmgMult: 20 }] },
  { id: 'x12', reqLv: 1, dmgMult: 1, type: 'main', effects: [{ type: 'hp_sacrifice', costPct: 0.5, dmgMult: NaN }] },
]);
...
const bad3 = validateSkillsDB([
  { id: 'x4', reqLv: 1, dmgMult: 1, type: 'instant', conditionMaxHPPct: 150, effects: [] },
  { id: 'x6', reqLv: 1, dmgMult: 1, type: 'main', conditionMaxHPPct: 0, effects: [] },
  { id: 'x7', reqLv: 1, dmgMult: 1, type: 'main', conditionMaxHPPct: -5, effects: [] },
]);
...
const bad4 = validateSkillsDB([{ id: 'x5', reqLv: 1, dmgMult: 1, type: 'main', priority: NaN, effects: [] }]);
t('priority 非数值被拒', bad4.errors.some((m) => m.includes('priority 非法')));
const bad5 = validateSkillsDB([{ id: 'x8', reqLv: 1, dmgMult: 1, type: 'main', effects: [null] }]);
t('effects 数组含非对象条目（null）→ 报告错误而非崩溃', bad5.errors.some((m) => m.includes('效果条目非法')), bad5.errors.join('; '));
const good = validateSkillsDB([
  { id: 'g1', reqLv: 1, dmgMult: 1, type: 'main', conditionMaxHPPct: 100, effects: [] },
  { id: 'g2', reqLv: 1, dmgMult: 1, type: 'dot', effects: [{ type: 'dot', dps: 0, dur: 1000 }] },
]);
...
console.log('== Skill 施放 ==');
const s01 = skills[0];
t('s01 use 恒 true（无使用限制）', s01.use() && s01.use());
const s07 = skills.find((s) => s.id === 's07');
t('s07 连续 use 恒 true（无使用限制）', s07.use() && s07.use());

console.log('== 类型与条件 ==');
t('主技能(main) 19 个（s39 终焉咏叹调改瞬发，2026-08-12）', skills.filter((s) => s.isMain).length === 19);
t('瞬发(instant) 10 个（s39 改瞬发）', skills.filter((s) => s.isInstant).length === 10);
...
t('效果类型全集 18 种（2026-08-12 敌人技能从零重做 +6：lost_hp_dmg/maxhp_dmg/atk_down/haste_down/heal_cut/multi_hit）', EFFECT_TYPES.length === 18 && EFFECT_TYPES.includes('ap_recover') && !EFFECT_TYPES.includes('mp_recover'));

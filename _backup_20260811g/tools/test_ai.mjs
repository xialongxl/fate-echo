...
console.log('== A. 敌人表 ==');
{
  ...
  const bossDomains = ENEMIES_DB.filter((e) => e.tier === 'boss').map((e) =>
    e.skills.some((s) => (typeof s === 'object' ? s : (ENEMY_SKILLS[s] || skillData(s)))?.type === 'domain'));
  ...
  const vamp = allEnemies().find((e) => e.id === 'e10').skills.find((s) => s.id === 'en_噬咬');
  ...
  const normalOgcd = ENEMIES_DB.filter((e) => e.tier === 'normal')
    .flatMap((e) => e.skills)
    .filter((s) => typeof s === 'string' && (ENEMY_SKILLS[s]?.type === 'ogcd' || skillData(s)?.type === 'ogcd'));
  ...
  const eliteHasInstant = ENEMIES_DB.filter((e) => e.tier !== 'normal').every((e) =>
    e.skills.some((s) => (typeof s === 'object' ? s : (ENEMY_SKILLS[s] || skillData(s)))?.type === 'ogcd'));
...
console.log('== B. 启发式 AI 行为 ==');
{
  ...
  t('B1 低血时优先治疗（en_再生 腐木再生）', heuristicDecide(engine, enemy, skills, rng)?.id === 'en_再生', `decide=${heuristicDecide(engine, enemy, skills, rng)?.id}`);
  ...
  t('B2 血健康时不用治疗（en_再生/en_愈合 降权）', d2?.id !== 'en_再生' && d2?.id !== 'en_愈合', `decide=${d2?.id}`);
  // 深渊领主 hp 55% → 自爆（s40 灵魂献祭：献祭 50% 后残血 5%，绝境搏命）
  const { engine: e3, enemy: en3, skills: sk3 } = vsEnemy('e11');
  en3.hp = Math.round(en3.maxHp * 0.55);
  t('B3 献祭后残血区间绝境自爆（s40 灵魂献祭）', heuristicDecide(e3, en3, sk3, rng)?.id === 'en_献祭', `decide=${heuristicDecide(e3, en3, sk3, rng)?.id}`);
  ...
  t('B3b hp 低于代价时不选自爆（防阻断发呆）', heuristicDecide(e3b, en3b, sk3b, rng)?.id !== 'en_献祭', `decide=${heuristicDecide(e3b, en3b, sk3b, rng)?.id}`);
  // hp 80% → 不自爆（sacrificeMid 低分，领域优先）
  const { engine: e4, enemy: en4, skills: sk4 } = vsEnemy('e11');
  en4.hp = Math.round(en4.maxHp * 0.8);
  t('B4 血充足时不用自爆', heuristicDecide(e4, en4, sk4, rng)?.id !== 'en_献祭');
  ...
  t('B5 狂暴后放弃自爆（非 en_献祭）', d5?.id !== 'en_献祭', `decide=${d5?.id}`);
  t('B6 狂暴后选高输出（en_虚空领域 180 分 > en_混沌箭）', d5?.id === 'en_虚空领域', `decide=${d5?.id}`);
  // 暗影刺客 hp 健康 → 易伤优先（s18）
  const { engine: e6, enemy: en6, skills: sk6 } = vsEnemy('e07');
  t('B7 玩家无易伤时优先上标记（en_标记）', heuristicDecide(e6, en6, sk6, rng)?.id === 'en_标记', `decide=${heuristicDecide(e6, en6, sk6, rng)?.id}`);
  // 玩家已有易伤 → 不重复标记，换高伤技（s25 混沌箭）
  e6.player.vulnTurns = 3; e6.player.vulnMult = 1.2;
  t('B8 玩家已有易伤时改用高伤技（en_混沌箭）', heuristicDecide(e6, en6, sk6, rng)?.id === 'en_混沌箭', `decide=${heuristicDecide(e6, en6, sk6, rng)?.id}`);
  ...
  en8.hp = en8.maxHp;
  t('B10 血满时治疗技降权（en_再生 不选，dot 仍用）', heuristicDecide(e8, en8, sk8, rng)?.id !== 'en_再生', `decide=${heuristicDecide(e8, en8, sk8, rng)?.id}`);
  // 瞬发选择：暗影刺客唯一 ogcd（s26 暗影裂隙）
  const { engine: e9, enemy: en9, skills: sk9 } = vsEnemy('e07');
  t('B11 精英瞬发选择（en_裂隙 暗影裂隙）', heuristicPickInstant(e9, en9, sk9, rng)?.id === 'en_裂隙');

...
console.log('== B. 版本迁移 ==');
{
  ...
  // v9 → v10：MP 删除（2026-08-11 用户定案，AP 回转体系）——player.mp/maxMp 丢弃，其余保留
  const v9s = sanitizeSave({ version: 9, player: { level: 7, hp: 88, mp: 42, maxMp: 100, skillLoadout: { regular: ['s01'], finale: ['s45'] } }, area: { depth: 3, areaIndex: 1 } });
  t('B1r v9→v10 迁移：mp/maxMp 删除、等级/血量保留、版本 10', v9s.version === 10 && !('mp' in v9s.player) && !('maxMp' in v9s.player) && v9s.player.level === 7 && v9s.player.hp === 88 && v9s.player.skillLoadout.regular.join() === 's01');

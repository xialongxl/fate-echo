...
export class BattleUI {
  ...
  _renderSkills() {
    ...
    const mkBtn = (skill, keyLabel, isInstant) => {
      ...
      if (skill.isFinale && !reason) btn.title = '终焉技能：消耗全部行动点（本回合独占行动）'; // 终焉大招提示（替代 CD）；禁用时保留具体原因
...
export class OverworldUI {
  ...
  renderSkillConfig(game) {
    ...
    // 技能描述模板填充（data.js desc 为末光毫秒制模板，2026-08-11 层数制/CD 简化后）：
    //   {dmg} 约伤害（无减伤近似）、{heal} 治疗、{shieldVal} 护盾；
    //   "持续{dur}层"（desc 已层数制）→ X 层；冷却模板已随彻底去 CD 删除（2026-08-11）；
    ...
    this.renderPanel(`⚙ 技能配置（常规 ${regularOn}/5 · 终焉 ${finaleOn}/3）`, (panel) => {
      ...
      const mkGroup = (title, list, sel, group) => {
        ...
        for (const s of list) {
          ...
          info.style.flex = '1';
          // 摘要行（倍率——战斗数值速览；MP/CD 已删，2026-08-11）+ 完整描述行（模板填充）
  ...
  renderGlossary(game, stats = null) {
    this.renderPanel('📖 术语表 · 帮助', (panel) => {
      ...
      const terms = [
        ...
        ['回合制', '持续效果为层数制（每回合 -1 层，减完结束）；技能无冷却，可每回合重复施放'],
  ...
  _renderPlayer(game) {
    ...
    //   局外/战斗显示一致）；急速 = AP 上限加成（每 10% +1 行动点上限，去 CD 后不再缩短 GCD）
    const u = makeBattlePlayerRef(game).unit;
...
// 宝珠属性名（详情显示）
const ORB_NAMES_REF = { hp_pct: '生命加成', atk_pct: '攻击加成', versa: '共鸣', crit: '暴击', finale_cd: '终焉回响' }; // finale_cd 效果已随去 CD 移除（2026-08-11）
// 词缀名（精炼详情显示）
const AFF_NAME_REF = { crit: '暴击率', haste: '急速', versa: '共鸣' }; // 急速 = AP 上限加成（去 CD 后非冷却缩减）
// 终焉图鉴收集数

...
export class CombatEngine {
  ...
  confirm() {
    ...
    for (const a of [...mains, ...instants]) {
      ...
      // 终焉消耗全部行动点（2026-08-11 定案，主/瞬发统一）：施放前置 0——
      //   效果内的 ap_recover（s45/s46 回满、s38 回 2）可在消耗后恢复（下回合保底留存）
      if (a.skill.isFinale) this.ap = 0;
  ...
  _cast(unit, skill) {
    ...
    // 2026-08-11 彻底去 CD：cd 全部为 0，use() 不置冷却（终焉回响宝珠的冷却缩减机制随 CD 删除）
    skill.use();
